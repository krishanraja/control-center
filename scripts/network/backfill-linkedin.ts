...
async function main() {
  ...
  for (const p of people) {
    ...
    // Did this actually produce the thing the run is for?
    const { data } = await sb.from('contacts').select('linkedin_url').eq('id', p.id).single()
    if ((data as any)?.linkedin_url) { resolved++; console.log(`  ✓ ${p.full_name}`) }
    else console.log(`  · ${p.full_name} — enriched, no profile found`)
  }

  console.log(`\n${resolved}/${ran} resolved to a real LinkedIn URL.`)
  console.log(`${ran - resolved} keep the search fallback, which still works.`)
