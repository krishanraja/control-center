-- Wave-2 fix (adversarial review, defer semantics): a DEFERRED task
-- (due_date in the future, krish_reviewed still false) leaves the day's
-- decision surfaces until its date arrives, then re-enters automatically.
-- Only the task branch changes; everything else identical to v2.
