...
--
-- Extend the decisions_waiting view with route_target so the client routes
-- by kind without if/else in components. The Home tab and Today tab both
-- pass (kind,id,route_target) into a single src/lib/routeDecision.ts helper.
--
-- Mapping:
--   task       → today?task=:id
--   idea       → content?idea=:id
--   guest      → guests?guest=:id
--   visibility → guests?target=:id
--   lead       → leads?lead=:id
--
-- Also: filter out content_ideas with state='dropped' which can leak in via
-- the seeded/researching/drafting/review band when a row is downgraded.
