-- decisions_waiting v4: acquisition approval kinds (Growth tab Phase 2)
--
-- Adds two branches to the canonical "what needs Krish" view:
--   - sequence_approval : acquisition_sequences status='proposed' — a typed,
--     only-Krish ruling (DECISION_KINDS in src/lib/decisionKinds.ts).
--   - send_sample       : acquisition_sends status='queued' AND sample_required
--     — batch queue work (QUEUE_KINDS), routed to the Growth tab deck.
-- Plus one exclusion on the task branch: the Nurture Scheduler's legacy
-- `send-{id}` approval tasks are hidden while their send row is queued, so a
-- send never surfaces twice (LOAD-BEARING during the cutover — ships in the
-- same migration as the new branches).
--
-- Column shape is copied verbatim from the live v3 definition
-- (kind, id, title, description, agent, status, priority, sort_at, url,
--  source_table, meta, route_target) — CREATE OR REPLACE requires it.
