...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // A declined market signal. The write must ride the service-role client here
  // (this route already does): zara_signals is anon-SELECT-only under RLS, so a
  // browser-side status write is a silent no-op — the same trap the promote
  // path hit before it moved server-side.
