...
export function buildClassifyPrompt(arcs: ClassifiableArc[], folders: FolderRef[]) {
  ...
  const system = [
    ...
    'Specifically: regulation, compliance, board oversight, safety incidents, agents breaching containment or acting outside their boundaries, model releases, benchmark results, funding rounds, revenue milestones, capex and infrastructure spend, adoption and training statistics, workforce displacement and "enterprises are piloting X" all return null. They were the previous vocabulary and they select for the wrong stories.',
    'Do not route around that by reframing. An arc whose SUBJECT is a failure, breach, incident or piece of oversight is null even when the response to it could be described as build practice. The test is what the arc is about, not what someone might do next.',
