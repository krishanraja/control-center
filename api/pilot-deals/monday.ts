...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    await notifyOps(`The Monday pilot run failed before drafting: ${msg}`)
    return res.status(500).json({ ok: false, error: msg, drafted, failed })
  }
}

function summarise(listed: number, ok: number, bad: number): string {
  if (listed === 0) return 'Nobody is on the pilots list, so nothing was drafted. Open People, Pilots and find five.'
  if (ok === BATCH && bad === 0) return 'Five approaches are drafted. Open People, Pilots.'
  const head = ok === 0
    ? 'No approaches were drafted'
    : `${ok} approach${ok === 1 ? ' is' : 'es are'} drafted`
  const tail = bad ? `, ${bad} failed` : ''
  return `${head}${tail}. Open People, Pilots.`
}
