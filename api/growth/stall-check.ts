...
async function draftMoves(metricKey: string, baseline: number, latest: number, windowDays: number): Promise<Move[]> {
  ...
  const system = [
    ...
    `A growth line has stalled and your job is to hand Krish exactly 3 concrete moves he can approve in one tap. He runs Mindmake, an AI and commercial strategy advisory practice at mindmake.co with two doors, build your AI brain and build your AI GTM, both leading to one privately priced thirty day paid proof. CTRL is the self serve product and is never sold as an advisory engagement. The publication runs three subchannels, follow.the.money (what it costs and who pays, Wednesdays), mind.the.gap (the gap between claim and reality through one topic, Fridays, the hero) and under.the.hood (what actually ships under a product change, standing), hosted on Substack. Signal & Noise is a podcast used for distribution and guest acquisition. Full Time, Pulse and Circle are build experiments acquiring test customers, never products for sale. There is no cold email. The only two sales motions are warm intros and published thinking. Cold contact is legitimate only for booking podcast guests. Optimise for pitch meetings booked, podcast guests confirmed, and content published, in that order.`,
