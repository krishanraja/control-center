-- network_search: let a question ask who is stuck, hiring or piloting.
--
-- The stance reaches the row and the retrieval text, and still could not be
-- retrieved: relationship value dominates the score by design, so the five
-- people actually stuck on an AI rollout never surfaced above eleven thousand
-- others. The signal was visible and unaskable.
--
-- intent_stance becomes a SOFT constraint, matched the same way seniority and
-- roles are. Soft rather than hard for the usual reason — an operator's typed
-- filter is a stated preference and a parsed one is a guess — and weighted at
-- the existing 0.22 constraint term rather than by re-tuning the five measured
-- top-level weights.
--
-- Replaced rather than dropped: the signature and return type are unchanged.
...
scored AS (
  SELECT
    ...
    ci.intent_score, ci.intent_stance, ci.intent_evidence, ci.intent_evidence_url,
        ...
        SELECT
          ...
          (CASE el->>'field'
            ...
            WHEN 'intent_stance' THEN CASE
              WHEN coalesce(ci.intent_score, 0) > 0 AND ci.intent_stance = ANY(vals) THEN 1
              ELSE 0 END
