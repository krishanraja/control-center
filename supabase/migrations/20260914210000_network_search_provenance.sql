-- network_search: return the provenance and the second channel with the row.
--
-- Two things the Network tab promises and could not keep before this migration,
-- both for the same reason — the answer was in `contacts` and the RPC never
-- carried it out:
--
--   "where do I know this person from"   origin_channel / origin_campaign /
--                                        first_met_context existed on every one
--                                        of the 10,768 rows and reached the UI
--                                        only through a per-person detail fetch
--                                        fired AFTER the sheet opened. A result
--                                        row could not show it at all.
--
--   "one click to their social"          twitter_handle was in the same
--                                        position, which is why the sheet had to
--                                        wait on /api/network/person/:id before
--                                        it could render an X button.
--
-- Both are plain columns on a table this function ALREADY joins. Adding them to
-- the projection costs nothing measurable and removes a network round-trip from
-- the path between deciding to contact someone and contacting them.
--
-- Dropped rather than replaced: the return type gains columns, and
-- CREATE OR REPLACE cannot change a function's result type. The signature,
-- the scoring and every filter are untouched — this is projection only.
