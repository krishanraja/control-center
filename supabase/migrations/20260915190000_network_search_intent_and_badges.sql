-- network_search: rank on what the record contains, and on who is talking now.
--
-- Two changes to actionability, both from running the enrichment for real.
--
-- 1. The hub term stops depending on a number we cannot get. The registered
--    profile actor returns no follower or connection count at all — confirmed
--    against its own 69-field list across 45 enrichments, and the defensive
--    parser did not change it because there was nothing to parse. Rather than
--    pay for a different actor, the term now takes whichever evidence of reach
--    we hold: Coresignal's real follower count where it exists, and otherwise
--    LinkedIn's own influencer and creator badges, which are scarce and which
--    mark the behaviour a follower count is a proxy for.
--
-- 2. Intent arrives. Someone publishing about AI this month is a reason to act
--    today, and this is the only term in the ranker that decays by itself. It
--    is capped well below the relationship terms deliberately: a stranger
--    posting about agents must never outrank someone Krish actually knows.
--
-- The five top-level weights are untouched. Everything here happens inside
-- actionability, whose components still sum to 1.
--
...
scored AS (
  SELECT
    ...
    ci.intent_score, ci.intent_topics, ci.intent_summary, ci.last_post_at,
    ...
    + (0.15 * (coalesce(ci.intent_score, 0)::numeric / 100))
    ) AS s_actionability,
...
SELECT
  ...
  intent_score, intent_topics, intent_summary, last_post_at,
...
COMMENT ON FUNCTION public.network_search IS
  'Hybrid semantic + lexical + constraint + relationship scoring over contact_intelligence. Constraints are SOFT (weighted partial credit); the only hard filters are is_person, do_not_contact, and explicit UI filters (tiers, roles, confidence, countries). Geography resolves through contact_intelligence.geo_code and pushes down into candidate recall. Actionability carries the hub and intent terms. Always returns rows unless a hard filter excludes everyone.';
