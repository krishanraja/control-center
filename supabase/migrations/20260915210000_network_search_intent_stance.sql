-- network_search: return the stance and the sentence behind it.
--
-- The intent columns were added to this function before the classifier grew a
-- stance, so it returned a score and no way to say what the score MEANT. The
-- row component reads intent_stance to choose its chip, which means the chip
-- could never render: the feature was live, correct in the database, and
-- invisible in the product.
--
-- It also returns the evidence quote, because the row's tooltip and the sheet's
-- pull-quote are the whole reason a person can overrule the classifier rather
-- than having to trust it.
--
-- Purely additive: no scoring change, no weight change. Dropped rather than
-- replaced only because the return type gains columns.
