-- The Network tab timed out again, and this time the retrieval text was not
-- the reason.
--
-- Live failure, 2026-09-23: "people in New York who work at large enterprises"
-- returned `network_search: canceling statement due to statement timeout`.
-- Measured on the production database, one call, warm cache:
--
--   lexical recall gate (c)   891 ms - 3,860 ms
--   semantic recall gate (b)  335 ms -   796 ms   (and see the note below)
--   relationship floor (e)     90 ms -   892 ms
--   soft geography (f)          3 ms -     8 ms
--   scoring + rescale         ~400 ms
--   ---------------------------------------------
--   whole call                1.3 s  -   7.8 s against an 8 s timeout
--
-- The same nodes swing by a factor of four between runs on an idle instance,
-- which is what makes this intermittent rather than dead: the search sits just
-- under the timeout and any load at all pushes it over. Two of those numbers
-- are structural and are what this migration removes.
--
-- ── 1. The lexical gate ranked the whole match set ─────────────────────────
-- Keywords are OR'd (20260810234500), so a five-word question matched 3,135 of
-- 11,755 people. The gate then read every one of those rows off the heap
-- (1,951 blocks) and ran ts_rank_cd - cover density, which walks lexeme
-- positions - over all of them, to keep 250.
--
-- Ranking 3,135 rows to keep 250 is the cost of perfect ordering in a stage
-- whose only job is to decide WHO GETS SCORED. The scorer downstream recomputes
-- the lexical signal properly for everyone in the pool, so ordering precision
-- here buys nothing; it only decides which keyword matches get a chance.
--
-- So the gate now ranks a bounded window: at most p_pool * 4 matches (1,000 at
-- the caller's settings), ordered by cover density within that window. Measured
-- on the same query: 891-3,860 ms becomes 21 ms, and the heap read drops from
-- 1,951 blocks to 583.
--
-- What this costs, stated plainly: for a query matching more than 1,000 people,
-- the 250 keyword candidates are drawn from the first 1,000 matches rather than
-- the best 1,000. That is the case where the lexical tier carries the least
-- information anyway - a term in a quarter of the corpus does not discriminate,
-- and the semantic, relationship and geography paths are all still exhaustive
-- over their own dimensions. For a query matching fewer than 1,000 people,
-- which is every narrow lexical query this tier exists for (a company name, a
-- surname, a product), the behaviour is IDENTICAL.
--
-- ── 2. The relationship floor seq-scanned the table ───────────────────────
-- Path (e) is the query-independent floor that makes "always return answers"
-- structural, and it has always been ORDER BY tier_weight DESC, warmth DESC
-- with no index to serve it: a full scan of 11,704 rows, 90 ms on a good run
-- and 892 ms on a bad one, on EVERY search. Path (f) does the same work behind
-- a geography predicate. Both get an index here.
--
-- ── Not fixed here, recorded because it is worth knowing ──────────────────
-- The semantic gate asks for p_pool (250) neighbours and gets 40. pgvector's
-- hnsw.ef_search defaults to 40 and caps the scan, so p_pool has never meant
-- anything to path (b). That also explains the 250-vs-400 benchmark recorded in
-- api/_networkSearch.ts: both returned the identical top twenty because both
-- were really 40. Raising it would change ranking and ADD latency, which is the
-- opposite of today's job, so it stays as it is and gets decided on its own.

-- ── The floor, and the floor behind a country ─────────────────────────────
CREATE INDEX IF NOT EXISTS ci_relationship_floor_idx
  ON public.contact_intelligence (tier_weight DESC, warmth DESC NULLS LAST)
  WHERE is_person;

CREATE INDEX IF NOT EXISTS ci_geo_relationship_floor_idx
  ON public.contact_intelligence (geo_code, tier_weight DESC, warmth DESC NULLS LAST)
  WHERE is_person;

-- ── The function, with a bounded lexical gate ─────────────────────────────
-- Unchanged from 20260916140000 except for union member (c).
...
cand AS (
  ...
  --
  --     BOUNDED, as of this migration. The inner LIMIT is the whole point: it
  --     stops the gate reading and cover-density-ranking every match in the
  --     corpus to choose 250 of them. See this file's header for what that
  --     costs and what it saves. The inner query deliberately has no ORDER BY,
  --     which is what keeps the LIMIT an optimisation barrier rather than a
  --     sort Postgres can push through.
  (SELECT z.contact_id
   FROM (
  ...
  --     asked. Served by ci_relationship_floor_idx as of this migration.
  (SELECT ci.contact_id
   FROM public.contact_intelligence ci
   WHERE ci.is_person
     AND ((SELECT geos FROM q) IS NULL OR ci.geo_code = ANY((SELECT geos FROM q)::text[]))
...
scored AS (
  SELECT
      ...
      FROM (
        SELECT
          ...
          (CASE el->>'field'
            ...
            -- Only a LIVE stance counts. intent_score decays to zero past a
            -- quarter, and matching a stale stance would answer "who is stuck
            -- on AI" with someone who was stuck last year.
    ...
    -- importer, and carrying a hub term and an intent term.
    --
    -- Where a follower count exists it is log-scaled and never read as
    -- connections_count: LinkedIn caps that at 500 for its "500+" display, so
    -- everyone consequential reports the same number and it separates nobody.
    -- Measured on the 169 Coresignal enrichments, followers ran 161 to 7,284+
    -- while connections_count was 500 for nearly all of them. ln() because the
    -- difference between 200 and 2,000 followers is real and the difference
    -- between 20,000 and 200,000 is mostly noise for this purpose.
    (
    ...
    -- Hub: whichever evidence of reach we actually hold. A follower count is
    -- the better measure and only Coresignal supplies one; the profile actor
    -- returns no follower or connection count at all, confirmed against its own
    -- field list across 45 enrichments. So the badges stand in where the number
    -- is missing. Influencer is LinkedIn's own scarce marker; creator means the
    -- person publishes, which is the behaviour a follower count is a proxy for.
    + (0.17 * greatest(
         ...
         CASE WHEN ci.is_influencer THEN 0.90
              ...
              ELSE 0 END))
    -- Intent: someone publishing about AI this month is a reason to act today,
    -- and this is the only term in the whole ranker that decays on its own.
    -- Capped well below the relationship terms on purpose — a stranger posting
