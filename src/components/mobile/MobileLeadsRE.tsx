...
function VentureChips({ venture, onPick }: { venture: string | null; onPick: (slug: string | null) => void }) {
  return (
    <div className="flex gap-2 overflow-x-auto scrollbar-hide -mx-1 px-1">
      <button
        onClick={() => onPick(null)}
        className={`px-3 py-2 rounded-full text-body font-medium whitespace-nowrap transition-colors ${venture === null ? 'btn-contrast' : 'bg-white/[0.06] text-ink-muted'}`}
      >
        All
      </button>
      {VENTURES.map(v => (
        <button
          ...
          className={`px-3 py-2 rounded-full text-body font-medium whitespace-nowrap transition-colors ${venture === v.slug ? 'btn-contrast' : 'bg-white/[0.06] text-ink-muted'}`}
        >
          {v.label}
        </button>
      ))}
      ...
      <div className="mt-4 flex-1 min-h-0 overflow-hidden">
        {fit && (
          <p className="text-body text-amber-200/90 leading-relaxed inline-flex items-start gap-1.5">
            ...
            <span><span className="text-ink-faint">Best fit: </span>{ventureLabel(fit.venture)} · {fit.score}</span>
          </p>
        )}
        {why && (
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className="flex-shrink-0 w-full rounded-2xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-ui text-ink placeholder:text-ink-faint focus:border-violet-400/40 focus:outline-none"
      />
      ...
      {handQueue.length > 0 && (
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <p className="text-micro text-ink-faint">One card at a time — right keeps, left skips with a reason.</p>
          </div>
        ...
        <FeedCard title={`Handle 1-by-1 · ${handQueue.length}`}>
          {handQueue.slice(0, 20).map(c => (
            <FeedRow
              ...
              trailing={
                <span className="flex items-center gap-1 text-body text-ink-faint tabular-nums">
        ...
        <FeedCard title={`Review · ${feed.length}`}>
          {feed.slice(0, 40).map(c => (
            <FeedRow
              ...
              trailing={<span className="text-body text-ink-faint tabular-nums">{humanAge(c.updated_at)}</span>}
              onClick={() => openLead(c)}
              feedback={{ sourceTable: 'contacts', sourceId: c.id, agentId: c.owner_agent }}
            />
          ))}
