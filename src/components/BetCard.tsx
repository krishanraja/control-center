    ...
    <article data-bet-id={bet.id} className={`rounded-xl border ${toneClass} p-3 transition-colors`}>
      <header className="flex items-start gap-2 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-micro text-ink-faint leading-snug mt-1 line-clamp-2">
            <span className="text-ink-faint">Wins if: </span>{bet.success_criterion}
          </p>
          <div className="flex items-center gap-2 mt-1.5 text-micro text-ink-faint flex-wrap">
      ...
      {bet.status === 'live' && (
        <div className="mt-2.5">
          <div className="flex items-center justify-between text-micro mb-1">
            <span className={overdue ? 'text-red-300 font-semibold' : 'text-ink-faint'}>
        ...
        <div className="flex items-center gap-1.5 mt-2.5 flex-wrap">
          ...
          {!overdue ? (
            <button
              ...
              disabled={busy !== null}
              className="flex items-center gap-1 px-2.5 py-1 rounded-md text-micro font-medium border border-white/10 text-ink-muted hover:bg-white/[0.06] disabled:opacity-40"
            >
              <Pause size={11} />
              Pause
            </button>
      ...
      {bet.learning && (
        <p className="text-micro text-ink-faint mt-2 leading-snug italic">
          <span className="text-ink-faint">Learning: </span>{bet.learning}
        </p>
      )}
        ...
        <div className="mt-3 rounded-lg border border-white/[0.12] bg-base p-3 space-y-2">
          ...
          {modal === 'extend' && (
            <input
              ...
              className="w-full bg-white/[0.04] border border-white/[0.08] rounded text-micro text-ink p-2 focus:outline-none focus:border-white/[0.18]"
            />
          )}
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={busy !== null}
              className="px-3 py-1 rounded-md text-micro font-medium text-ink-faint hover:text-ink-muted"
            >
              Cancel
            </button>
          </div>
