...
export function NextBestActionHero({ ideas, narrow }: Props) {
  ...
  const markPublished = async (id: string, url: string) => {
    ...
    try {
      ...
      if (r.status === 409 && (body?.reason === 'fact_gate' || body?.reason === 'publish_gate')) {
    ...
    pubOpen ? (
      <div className="flex-shrink-0 flex items-center gap-1.5">
        <input
          ...
          className="w-40 rounded-lg bg-sunk border border-white/12 px-2.5 py-2 text-label text-ink placeholder:text-ink-faint focus:outline-none focus:border-emerald-400/40"
        />
        <button
