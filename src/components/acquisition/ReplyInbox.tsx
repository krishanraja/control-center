...
const CLASS_TONE: Record<string, string> = {
  ...
  unsubscribe: 'text-ink-faint',
  auto_reply: 'text-ink-faint',
  ...
  return (
    <section className="rounded-xl border border-white/[0.07] bg-white/[0.015] overflow-hidden">
      <header className="px-4 py-3 flex items-center gap-2 border-b border-white/[0.06]">
        ...
        <span className="ml-auto text-micro text-ink-faint tabular-nums">{open.length} open</span>
      </header>
          ...
          {open.slice(0, 6).map(r => (
            <div key={r.id} className="px-4 py-2.5">
              <div className="flex items-center gap-2">
                <span className="text-label font-medium text-ink truncate">{r.from_email || '(unknown)'}</span>
                <span className={`text-micro uppercase tracking-wide flex-shrink-0 ${CLASS_TONE[r.classification] || 'text-ink-faint'}`}>
              ...
              {(r.subject || r.body) && (
                <p className="text-micro text-ink-faint truncate mt-0.5">{r.subject || r.body}</p>
              )}
              <div className="flex gap-2 mt-1.5">
                ...
                <button
                  ...
                  className="rounded-lg border border-white/[0.1] px-2.5 py-1 text-micro font-medium text-ink-faint hover:text-ink-muted transition-colors disabled:opacity-40"
                >
                  Close
                </button>
              </div>
