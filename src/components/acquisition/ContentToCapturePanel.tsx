  ...
  return (
    <section className="rounded-xl border border-white/[0.07] bg-white/[0.015] overflow-hidden">
      <header className="px-4 py-3 flex items-center gap-2 border-b border-white/[0.06]">
        ...
        <span className="ml-auto text-micro text-ink-faint tabular-nums">{nonEmpty.length} converting</span>
      </header>
        ...
        <div className="divide-y divide-white/[0.04]">
          {nonEmpty.slice(0, 8).map(r => (
            <div key={r.id} className="px-4 py-2.5">
              <p className="text-label text-ink-muted truncate">{r.idea || r.utm_campaign}</p>
              <div className="flex items-baseline gap-3 mt-0.5 text-micro tabular-nums">
                <span className="text-ink-faint truncate">{r.utm_campaign}</span>
                <span className="ml-auto text-ink-faint flex-shrink-0">{r.captures} captured</span>
                <span className={`flex-shrink-0 ${r.paid > 0 ? 'text-emerald-300' : 'text-ink-faint'}`}>{r.paid} paid</span>
