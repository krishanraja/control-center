...
export function TabChatHost({ tab, narrow, params, suppressed = false }: TabChatHostProps) {
  ...
  const body = (
    <div className="flex flex-col gap-3 px-4 pb-[calc(env(safe-area-inset-bottom,0px)+16px)] pt-1">
      {history.length === 0 && (
        <p className="font-serif text-lede italic text-ink-faint">
      ...
      {history.map(ex => (
        <div key={ex.id} className="space-y-1.5">
          <div className="flex items-start gap-2">
            <span className="mt-0.5 flex-shrink-0 text-micro uppercase tracking-[0.14em] text-ink-faint">You</span>
    ...
    <>
      ...
      {!suppressed && !narrow && (
        <button
          ...
          className="tab-chat-pill surface fixed bottom-[4.25rem] right-5 z-30 hidden items-center gap-2 rounded-full px-3 py-2 text-ink-muted shadow-e2 backdrop-blur transition-colors hover:text-ink min-[900px]:flex"
