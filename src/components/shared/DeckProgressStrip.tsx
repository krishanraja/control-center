...
export function DeckProgressStrip({
  ...
  return (
    <div className="flex items-center gap-2 pt-1 pb-3 flex-shrink-0">
      ...
      <span className="text-label text-ink-muted font-medium">{title}</span>
      <span className="text-label text-ink-faint tabular-nums">· {remaining} left{triagedCount > 0 ? ` · ${triagedCount} cleared` : ''}</span>
      {(canUndo || onExit) && (
        <div className="ml-auto flex items-center gap-1.5">
          ...
          {onExit && (
            <button type="button" onClick={onExit}
              className="inline-flex items-center gap-1.5 text-label font-semibold text-ink-muted border border-white/20 bg-white/[0.06] hover:bg-white/[0.12] active:scale-95 transition px-3 py-1.5 rounded-lg">
