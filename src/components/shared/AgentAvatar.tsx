...
const SIZE_MAP: Record<Size, { box: string; text: string; dot: string; ring: string }> = {
  xs: { box: 'w-6 h-6',    text: 'text-micro', dot: 'w-1.5 h-1.5', ring: 'ring-1' },
  sm: { box: 'w-8 h-8',    text: 'text-label', dot: 'w-2 h-2',     ring: 'ring-1' },
  md: { box: 'w-10 h-10',  text: 'text-ui', dot: 'w-2.5 h-2.5', ring: 'ring-2' },
  lg: { box: 'w-14 h-14',  text: 'text-title', dot: 'w-3 h-3',     ring: 'ring-2' },
  xl: { box: 'w-20 h-20',  text: 'text-heading', dot: 'w-3.5 h-3.5', ring: 'ring-2' },
}
