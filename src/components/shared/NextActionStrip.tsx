...
export function NextActionStrip({
  ...
  return (
    <div className="flex items-center gap-4 rounded-xl border border-white/[0.07] bg-gradient-to-r from-white/[0.025] to-transparent px-4 py-3">
      ...
      <p className="text-[13px] text-white/70 flex-1 min-w-0 truncate" title={insight}>
        {insight}
      </p>
      ...
      <button
        ...
        onClick={onCta}
        disabled={disabled}
        className="flex-shrink-0 flex items-center gap-1 px-3 py-1.5 rounded-md text-[12px] font-semibold bg-violet-500/90 text-white hover:bg-violet-400 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
      >
        {ctaLabel}
