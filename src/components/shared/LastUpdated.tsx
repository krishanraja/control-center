...
export function LastUpdated({ date, refreshing = false }: { date: Date | null; refreshing?: boolean }) {
  if (refreshing) {
    return (
      <span className="inline-flex items-center gap-1 text-micro text-ink-faint font-mono" role="status" aria-live="polite">
        <Working size={10} />
        Checking
      </span>
    )
  ...
  if (!date) return null
  return (
    <span className="inline-flex items-center gap-1 text-micro text-ink-faint/50 font-mono">
      <Clock size={10} />
