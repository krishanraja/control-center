      ...
      <div className="lg:col-span-3 space-y-3">
        ...
        <div className="rounded-xl border border-white/[0.07] bg-white/[0.015] p-4 space-y-3">
          <label className="block">
            <span className="text-micro font-semibold uppercase tracking-[0.14em] text-ink-faint">Transcript</span>
            <textarea
              value={transcript}
              onChange={e => setTranscript(e.target.value)}
              placeholder={PLACEHOLDER_TRANSCRIPT}
              rows={8}
              className="mt-1.5 w-full rounded-lg border border-white/[0.08] bg-sunk px-3 py-2 text-label text-ink-muted placeholder-white/25 focus:outline-none focus:border-violet-500/40 resize-y"
            />
            <span className="text-micro text-ink-faint mt-1 block">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <label className="block">
              <span className="text-micro font-semibold uppercase tracking-[0.14em] text-ink-faint">Client name</span>
              <input
                value={clientName}
                onChange={e => setClientName(e.target.value)}
                placeholder="e.g. Acme Co"
                className="mt-1.5 w-full rounded-lg border border-white/[0.08] bg-sunk px-3 py-2 text-label text-ink-muted placeholder-white/25 focus:outline-none focus:border-violet-500/40"
              />
            </label>
            <label className="block">
              <span className="text-micro font-semibold uppercase tracking-[0.14em] text-ink-faint">Client email</span>
              <input
                value={clientEmail}
          ...
          <button
            ...
            className="text-micro text-ink-faint hover:text-ink-muted transition-colors"
          >
          ...
          {showAdditional && (
            <textarea
              ...
              className="w-full rounded-lg border border-white/[0.08] bg-sunk px-3 py-2 text-label text-ink-muted placeholder-white/25 focus:outline-none focus:border-violet-500/40 resize-y"
            />
          )}
          ...
          {triage && !triage.passed && (
            <div className="rounded-lg border border-amber-500/25 bg-amber-500/[0.06] px-3 py-2 text-label text-amber-200/85">
              ...
              <span className="ml-1.5">Better as a <code className="text-ink-muted">{humanizeRoute(triage.result)}</code>.</span>
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            {(transcript || clientName) && !generating && (
              <button
                onClick={resetForm}
                className="text-label text-ink-faint hover:text-ink-muted transition-colors px-2"
              >
                Clear
              </button>
            )}
