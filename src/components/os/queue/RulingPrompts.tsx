...
export function useRulingPrompts() {
  ...
  const overlay = active == null ? null : (
    ...
    >
        {active.type === 'note' ? (
          <>
            <div className="text-body font-semibold text-ink-muted">Send back with a note</div>
            <div className="text-micro text-ink-faint truncate">{active.title}</div>
            <textarea
              autoFocus
              value={note}
              ...
              className="w-full bg-sunk border border-white/10 rounded-xl px-3 py-2 text-body text-ink-muted focus:outline-none focus:border-white/25 resize-none"
            />
            <div className="flex justify-end gap-2">
              <button onClick={() => settle(null)} className="px-3 py-1.5 rounded-lg text-label border border-white/10 text-ink-faint hover:text-ink-muted">Cancel</button>
              <button
                onClick={() => settle(note.trim() || null)}
                disabled={!note.trim()}
                className="px-3 py-1.5 rounded-lg text-label font-semibold bg-white/10 border border-white/15 text-ink hover:bg-white/15 disabled:opacity-40"
              >
                Send back
              </button>
            </div>
          ...
          <>
            <div className="text-body font-semibold text-ink-muted">Defer until</div>
            <div className="flex flex-col gap-2">
              {DEFER_CHOICES.map(c => (
                <button
                  key={c.key}
                  onClick={() => settle(c.key)}
                  className="w-full rounded-xl py-3 text-body font-semibold bg-white/[0.06] text-ink-muted border border-white/10 hover:bg-white/[0.1]"
                >
                  {c.label}
                </button>
              ))}
              <button onClick={() => settle(null)} className="w-full rounded-xl py-2.5 text-label text-ink-faint hover:text-ink-muted">Cancel</button>
            </div>
          </>
        )}
    </Modal>
