...
describe("property API composition", () => {
  ...
  it("maps database columns to browser keys and coerces numerics", () => {
    const day = composePropertyDay({
      property: { id: "p", slug: "s", label: "L", address: "A", suburb: "S", state: "QLD", postcode: "4101", dwelling_type: "unit", bedrooms: "2", bathrooms: 2, car_spaces: 1, purchase_price_aud: "605000", settled_on: "2024-11-14" },
      loans: [{ id: "l", lender: "B", purpose: "investment", principal_aud: "484000", term_months: 360, repayment_type: "principal_and_interest", first_repayment_on: "2024-12-13", repayment_aud: "3079.49", offset_balance_aud: "0" }],
      rates: [{ effective_from: "2024-12-13", rate_pct: "6.56", source: "settlement" }],
      rents: [],
      valuations: [{ estimated_on: "2024-11-14", method: "purchase_price", mid_aud: "605000", confidence: "high", inputs: {}, engine_version: "v" }],
      ledger: [{ occurred_on: "2025-03-08", sheet_category: "Mortgage", category: "loan_repayment", direction: "out", amount_aud: "3079.49", synced_at: "2026-09-04T00:00:00Z" }],
      observations: [],
    ...
    expect(day.property.purchasePriceAud).toBe(605000);
    expect(day.loan.repaymentAud).toBe(3079.49);
    expect(day.rates[0].ratePct).toBe(6.56);
    expect(day.valuations[0].lowAud).toBeNull();
    expect(day.ledger[0].amountAud).toBe(3079.49);
