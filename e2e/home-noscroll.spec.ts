...
async function mockHome(page: Page, state: 'empty' | 'full') {
  ...
  const targets = { approaches_sent: 25, calls_taken: 5, paid_pilots: 1, cash_invoiced_gbp: 15000, pieces_published: 12, unasked_hours: 0 }
  const totals = full
    ? { approaches_sent: 13, calls_taken: 2, paid_pilots: 1, cash_invoiced_gbp: 15000, pieces_published: 4, unasked_hours: 12.5 }
  ...
  await page.route('**/api/scorecard*', (r: Route) => r.fulfill({ json: {
    ...
    current: { week_ending: '2026-09-25', approaches_sent: full ? 3 : 0, calls_taken: 0, paid_pilots: 0, cash_invoiced_gbp: 0, pieces_published: 1, unasked_hours: full ? 12.5 : 0, unasked_measured: full, commits: full ? 25 : 0 },
    weeks: [
    ...
    targets,
    totals,
    gap: { approaches_sent: 25 - totals.approaches_sent, calls_taken: 5 - totals.calls_taken, paid_pilots: 1 - totals.paid_pilots, cash_invoiced_gbp: 15000 - totals.cash_invoiced_gbp, pieces_published: 12 - totals.pieces_published, unasked_hours: totals.unasked_hours },
