...
- `bg-white/NN` and `border-white/NN` are theme-adaptive tints and hairlines.
  ...
  uses `text-ink`, `text-ink-muted` or `text-ink-faint` so hierarchy is
  explicit. **One spelling, since 2026-09-13.** The hierarchy used to have two
  names for the same three channels: the `text-strong / text-muted /
  text-faint` component classes in `index.css` and the `text-ink*` Tailwind
  tokens. Neither won. 23 call sites used the class spelling, 21 files used the
  tokens, and 185 of 257 component files ignored both and stayed on
  `text-white/NN` opacity soup, which is most of why the app reads as several
  type systems. The tokens win because they take opacity modifiers and variants
...
**The claim is one primitive.** `<Claim>` (`components/shared/Claim.tsx`) is
THE serif payoff line: `font-serif`, a role size, `leading-[1.4]`, `text-ink`,
with an optional `source` line in micro underneath. Newsreader is reserved for
the claim or the earned payoff, one per surface, and `<Claim>` is how it is
spelled. Before it, 20 serif call sites carried six different recipes
(`leading-snug`, `leading-relaxed`, `leading-[1.35]`, `tracking-tight`, and
colours at `white/92`, `/85`, `/80`, `/75`, `/40`) for what is one idea. Focus
& Purpose ran the correct recipe from the day it shipped, which is why it is
the surface whose type reads right; `<Claim>` is that recipe, extracted. It is
never clamped: `compact` steps the size down a rung on short phone viewports
rather than cut the sentence. Serif copy that runs to paragraphs is a different
job and uses `font-body`, not this.
...
- **Copy the product renders and work the product proposes are two registers.**
  Everything above governs chrome: labels, buttons, empty states, waits. It
  does not govern what the OS puts in front of him to make. A prompt that
  proposes work is built from a claim rule and then a wall of prohibitions, and
  a model given only bans returns correct, joyless work, which is not something
  anyone opens twice (Krish, 2026-09-13: "the content suggestions are so
  serious and intense"). Proposal prompts carry `proposalPlay()` from
  `api/_humor.ts`: it says what to reach for, not only what to avoid, and
