...
1. **The Weekly Brief** — one synthesised roundup in the Mindmaker LIVE shape (Headlines / What this actually means / Perspectives), written for business leaders, assembled Friday 18:00 UTC, edited in the Composer, fanned out to multiple channel formats in one push, sent Monday.
...
| R2 | Destinations: Mindmaker LIVE newsletter + LinkedIn (per-week selection; Builder Economy IG optional). *(Amended 2026-08-06: the Techonomic essay destination is retired. Techonomic is folded into Mindmaker LIVE, and its long-form register survives as the "Full essay" format, not a separate channel.)* |
...
| `api/briefs/assemble.ts` | cron Fri 18:00 UTC | Cluster the week's items (existing embedding clusterer), pick headline set balanced across lanes, write per-story why-lines, draft the "What this actually means" essay grounded in the shifts register + krish-voice + content corpus, seed Perspectives from Krish's week (bets/decisions/audit_log highlights). Writes `weekly_briefs` (status `ready`, version 1) + `brief_review` decision + `purge_preview` decision + graduation candidates (evergreen-horizon items cited 3+ times). |
...
- **Mobile (one-thumb contract, R13)**: decision queue one-card-at-a-time with progress + time estimate; brief section-by-section read mode with magic row (Tighten / Sharper open / Harder ending / More data) previewing inline via the span-rewrite engine + "Tell Cleo" dictation (Web Speech API, degrade to text sheet); ship screen with big format checkboxes + one push. Composing/restructuring is desktop-only by design.
