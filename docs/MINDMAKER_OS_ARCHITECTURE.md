...
**The card-level controls (`src/components/content/ContentEnginePanel.tsx`, rendered inside `ContentIdeaCardActionable`).** Operates on whichever row it is handed — a parent idea or a transformed child — using that row's `body` as the working draft:
...
**New `/api/content-ideas/:id/*` routes.** `revise` (transform axes + iterate, voice-grounded, preview-only), `challenge` (tiered enrich → `meta.challenges[]`), `score` (Five Standards → `meta.standards` + `quality_score`; optional `model` param), `push-to-cleo` (server-side proxy to the content-factory webhook). Shared helpers in `api/_content.ts` (Claude / voice / corpus / CORS). The content-factory webhook URL is server-side + rotatable (`N8N_CONTENT_FACTORY_WEBHOOK_URL`).

**Schema.** Zero new tables — `content_ideas.meta` (jsonb) absorbs `revisions` / `challenges` / `standards` / `cleo_pushes`, alongside the existing `transformed_outputs` and `quality_score`. `meta` is the durable home for engine state.
