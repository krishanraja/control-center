...
**Mobile is review-first, not deep work (`src/components/mobile/MobileContent.tsx` + the Composer's `narrow` body).** A phone is for reviewing what is next or urgent, making a quick magic adjustment, previewing, and pushing — not careful writing. Mobile Content is a **"Ready for you" deck** showing only pieces awaiting Krish's sign-off (`review`), ready to ship (`approved`), or urgent (scheduled today / overdue / `cadence_due_at` due); upstream states, the seed rail, and the lane grid are hidden. Opening a piece on mobile renders the draft in read mode (no keyboard) with a one-tap magic row (Tighten / Sharper open / Harder ending / Make it ready) that previews inline, Cleo / Materials / Research as secondary sheets, and a big sticky Save Draft. The desktop deep-work surface is unchanged.
