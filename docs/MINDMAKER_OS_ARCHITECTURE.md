...
| `growth_touchpoints` | The ICP touchpoint map, the Growth spine (30 seeded rows across 5 products): `product_slug`, `icp_trigger`, `channel` (seo/geo/social_organic/social_paid/substack/partner/community/product/podcast/maven), `watering_hole`, `cost_efficiency_score` (1-10, rescored by the council), `coverage_status` (unaddressed/in_progress/covered/retired), `owner_agent`, `rationale`, `evidence` jsonb (answered assumptions land in `evidence.resolved_assumptions`), `assumption_flag` (the open question the row is still guessing at). `content_ideas.touchpoint_id` FKs here |
...
| `growth_council_reviews` | Weekly growth council output, one row per product per week (unique on week_start + product_slug): `findings` jsonb, `kill_list`, `double_down`, plus Krish's `krish_decision` and `decided_at` written from the Growth tab |
...
| **Growth** | Four sections over one spine (§5.9): **Touchpoint map** (the ICP touchpoint map per product, inline-editable `coverage_status` + `cost_efficiency_score`, add/retire, `assumption_flag` rendered as an answerable open question, plus the channel-accounts strip), **Creative** (the Higgsfield kanban: brief / script / producing / produced / posted, drag or arrow moves, script + shot notes + asset/posted URLs on the card, weekly 3 to 5 batch cap counted from `batch_week`), **Council** (weekly reviews newest first with findings / kill list / double down, Krish records `krish_decision`), **GEO** (probe results grouped by product with the citation rate as the headline number). Council and GEO show honest empty states while their tables are empty. | `growth_touchpoints`, `growth_creative_queue`, `growth_council_reviews`, `growth_geo_probes`, `growth_social_accounts` |
...
| **`/api/growth/touchpoints`** | GET / POST / PATCH | The Growth tab spine: read the ICP touchpoint map, add a touchpoint, edit coverage / cost efficiency / owner inline, retire (`coverage_status='retired'`), and `action:'answer_assumption'` which files an answer into `evidence.resolved_assumptions` and clears the open question |
