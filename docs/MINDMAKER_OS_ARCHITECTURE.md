...
1. **Mindmaker OS is a fleet of AI agents that runs Krish Raja's business portfolio** — content + products: Mindmaker as missionary vehicle, content channel, and build-lab; builder products (Fractionl, CTRL, Plinth, Full Time); and content brands (Builder Economy, Signal & Noise) — so Krish spends his hours on decisions, not admin.
...
- **Transform axes** — one-click tone (punchier / contrarian / warmer / formal), length (short LinkedIn / mid Mindmaker Live / full essay), and zoom-into-sharpest-angle. Preview-then-accept: the row's `body` is not mutated until Krish accepts; a non-destructive history lands in `meta.revisions[]`.
...
| `content-corpus/SKILL.md` | The channel corpus — **mandatory companion to krish-voice for any named channel or outbound**. The *what, who, and how-good*: per-channel mandate (Builder Economy / Signal & Noise / Mindmaker LIVE, including the long-form investigation format), the lead + visibility overlay, and the Five Standards gate (undeniably unique, well-researched, thoughtful, kind, helpful). The VPS copy still titles the investigation playbook after the retired Techonomic brand; `api/_content.ts` matches both headings until it is rewritten |
...
| **Mindmaker LIVE** | mindmakerlive.substack.com | The publication. Weekly Headlines / Perspectives / Resources, plus the long-form **investigation** (the teardown: take a claim, decompose it, verify each part against dated evidence, publish where the knowable ends) |
| **Personal Brand** | (LinkedIn / X) | Cleo's content engine target #1 |

**Retired from the OS content plane 2026-08-06 (Krish directive):** **Techonomic**. The brand, the `techonomic.co` destination (which never had a production deployment and served a 409 behind a failed TLS handshake), the `techonomic` lane, and the `techonomic` factory channel are all gone. Everything folds into **Mindmaker LIVE**: fewer brands, and the investigative depth engine is the public version of the advisory offer, so the proof belongs where the offer lives. Techonomic's remit was a topic, not an identity. What survives is the **format**, not the channel: the "Full essay" transform axis, the `investigation` venture rubric in `api/_finalPass.ts` (five lenses, an unverifiable load-bearing claim is an instant fail), and the whole investigation pipeline (`api/_investigation.ts`, `api/_gates.ts`, `api/_harness.ts`, `api/investigations/*`), which now publishes to Mindmaker LIVE. `tech0nomic.substack.com` and its subscribers still exist and are labelled *(retired)* wherever they surface; Krish owns the Substack-side migration.

**Channel mandates** for the content brands + the lead/visibility outbound overlay live in `/root/.openclaw/skills/content-corpus/SKILL.md` (companion to `krish-voice`). Cleo / Nell / Nova load it before composing for a named channel: it defines what each channel is *for* (Builder Economy = the why beneath the why; Signal & Noise = no-BS devil's-advocate; Mindmaker LIVE = weekly Headlines/Resources/Perspectives + why-it-matters, plus the long-form investigation of how the digital world gets paid for) and the Five Standards gate every piece must clear.
...
- **Not done here.** The `system_config.content_corpus` playbook is still headed "Techonomic (techonomic.co)" and `system_config.content_lane_techonomic` still exists (DB content, Krish's call); `api/_content.ts` matches both the old and the neutral headings so the depth engine keeps its playbook either way. Four n8n workflow definitions were updated in git and need `scripts/n8n/sync.sh --apply` to reach cloud.
