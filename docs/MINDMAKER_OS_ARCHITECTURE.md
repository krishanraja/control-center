...
- **Iterate** — quick chips (shorter / sharper hook / more data / harder ending) + an open feedback box. Same preview loop. The `/revise` API also accepts a `selection` for paragraph/sentence-level rewrites.
- **Challenge / enrich** — Challenge this, Counter-argument, Commercial hook, Add sources. Tiered research (Perplexity counter-evidence + a practitioner/community read + optional NewsAPI dated proof) synthesised by Claude into steelman → counter → sharper-take → hook, written to `meta.challenges[]`. Sourced, never invented.
...
- **Push to Cleo** — fires the live `Cleo | Mindmaker OS | Omnichannel Content Factory` webhook (`/webhook/content-factory`, workflow `AnhkJrJBvmohfqjJ`) with `{target_channel, hook, target_audience, contrarian_angle}`; the factory assembles a channel-specific draft, writes a Google Doc into the channel's Drive folder, and pings Krish on Telegram. Drafts only — nothing auto-publishes.

**Auto-seed (`src/components/content/ContentSeedRail.tsx`).** Top-of-tab rail offering this week's real artifacts (recent `zara_signals` + `customer_contacts` voice) as one-click seeds into `content_ideas` — builder-operator artifacts beat a blank box.
...
**Feature flag.** `VITE_CONTENT_ENGINE_ENABLED` (build-time; set true in Vercel env + redeploy to go live). Server keys: `N8N_CONTENT_FACTORY_WEBHOOK_URL`, `PERPLEXITY_API_KEY`, `ANTHROPIC_API_KEY`, optional `NEWSAPI_KEY`.
