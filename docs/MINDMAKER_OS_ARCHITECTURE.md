...
- venture_registry: new live rows `mindmake` and `publication`; the old `mindmaker` and `mindmaker_live` rows kept and marked inactive so historical rows still resolve. `signal_noise` reactivated as a channel. FK children and every venture/lane/product column migrated. `customer_product` enum values renamed in place.
- Content taxonomy: the publication now runs exactly two channels, **The Money of AI** and **Built with AI**. `venture_formats` and `content_ideas.lane_slot` moved to `money_of_ai` / `built_with_ai`, with canon mandates written into `venture_formats.mandate`. The live `system_config.content_corpus` headings were renamed to match, and the slicing was verified against the live corpus for all five channels.
- The offer ladder is retired. Two doors, one privately priced thirty-day paid proof, no public price, no diary link.

**What was deliberately NOT changed, because it is infrastructure and not a name.**
`krish@themindmaker.ai` (the only published contact address; the mailbox migration has not been done), `live.themindmaker.ai`, `mindmakerlive.substack.com`, `maven.com/mindmaker`, the Vercel project `control-center-mindmaker`, the webhook path `/webhook/mindmaker-orchestrator`, `Mindmaker LLC` and the `mindmaker_llc` Stripe account key. A blind rename broke several of these during this pass and each was restored and re-verified. **Adtech and martech history, contacts and network stay fully live** and are the warm network the pitch motion runs on.

**Acquisition doctrine, restated.** No cold email. The only two sales motions are warm intros and published thinking. Cold contact is legitimate for exactly one purpose, booking podcast guests, which is an ask and not a sale. The OS optimises for, in priority order: pitch meetings for Mindmake, podcast guests confirmed, content published. Agent autonomy was raised so guest booking, warm-intro surfacing and meeting prep run without Krish; the authority boundary is unchanged and nothing is sent, published or scheduled without per-asset approval.

**Two live defects found and fixed while doing this.**
...
2. `sync-briefs-to-skills.sh` runs Google Doc to SKILL.md to Supabase, and the Docs were far thinner than the Supabase briefs (cleo 7.5k against 28.6k, zara 2.5k against 21.4k). An unguarded run **deletes the richer brief and resurrects retired brands**, which is why the 2026-08-06 purge kept coming back. A **revert guard** was added: it refuses when the Doc would drop the canon block or shrink the brief by more than 20 percent, overridable with `FORCE_BRIEF_SYNC=1`. Verified: it blocks zara. The five doc-backed Docs still need updating from Supabase, which needs Krish, and until then that guard is what protects the briefs.
