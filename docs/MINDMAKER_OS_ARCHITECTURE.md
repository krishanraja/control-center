...
> 2. **control-center GitHub repo:** `docs/MINDMAKER_OS_ARCHITECTURE.md`. The VPS clone at `/root/Projects/control-center/docs/...` is the same surface; the sync script writes that path directly, so it can be current while GitHub is behind. **Push, or GitHub silently lags.**
...
1. **Mindmaker OS is a fleet of AI agents that runs Krish Raja's business portfolio** - content + products: Mindmaker as missionary vehicle, content channel, and build-lab; builder products (Fractionl, CTRL, Legibility, Full Time); and content brands (Built, Signal & Noise) - so Krish spends his hours on decisions, not admin.
...
**Brands (REFOCUSED 2026-08-11).** `themindmaker.ai` is the business: the Mindmaker advisory, selling two engagements (The Handover, The Teardown). `live.themindmaker.ai` is **Mindmaker Live**, the content arm, and it carries paid subscription tiers so it is never described as free. `makeyourmindup.ai` is **the CTRL app** and "Mindmaker Live" is the lead magnet into it, a PRODUCT surface and no longer a content brand at all (`ctrl.themindmaker.ai` 308s there).

**Mindmaker Live formats: there are exactly two.** `Paid` (hero, the investigation, carrying the retired Techonomic register, corpus key `paid`) and `Built` (builder conversations, carrying the builder economy thesis, corpus key `built`).

**Retired as ventures on the same day.** *Builder Economy* is fully retired, feed and back catalogue included. *Signal & Noise* is demoted from venture to **distribution channel**: nothing is commissioned for it, it carries Mindmaker Live material, and its feed, GUID and subscribers are deliberately untouched with no public repositioning made. The `Mindmaker Live` weekly format is retired, because that name is now the CTRL door.
...
| `content-corpus/SKILL.md` | The channel corpus - **mandatory companion to krish-voice for any named channel or outbound**. The *what, who, and how-good*: per-channel mandate (Built / Signal & Noise / Mindmaker Live, including the long-form investigation format), the lead + visibility overlay, and the Five Standards gate (undeniably unique, well-researched, thoughtful, kind, helpful). The investigation playbook was retitled *Investigation (long-form teardown)* in `system_config.content_corpus` on 2026-08-06; `api/_content.ts` still matches the legacy heading so an older copy keeps resolving. Two-way collision rule: that section must never be retitled to contain "Mindmaker Live" (it precedes section 4 and would capture the `makeyourmindup` lookup), and the weekly Mindmaker Live section must never be titled with "Teardown" or "Investigation" (the format is called *Mindmaker Live: Paid*, so this is now easy to get wrong, and the investigation pattern is tested first) |
...
|---|---|---|
| **Mindmaker Live** | live.themindmaker.ai (301 to mindmakerlive.substack.com) | THE content venture, `mindmaker_live` in venture_registry. Two formats: **Paid** (the investigation: take a claim, decompose it, verify each part against dated evidence, publish where the knowable ends) and **Built** (builder conversations). Carries paid tiers, so never call it free, and always link the branded domain |
| **Signal & Noise** | (podcast) | A **distribution CHANNEL**, not a venture, since 2026-08-11. AI in media; co-hosted with Rio Longacre + Brett House. Carries Mindmaker Live material. Feed, GUID and subscribers deliberately untouched; no public repositioning made |
...
| ~~Personal Brand~~ | (LinkedIn / X) | Superseded by Mindmaker Live. LinkedIn and X are channels |
...
**Where it lives now (2026-08-11).** That register ships as **Paid**, one of the two Mindmaker Live formats. It folded into MYMU on 2026-08-06 and moved again with everything else when MYMU became the CTRL lead magnet. `tech0nomic.substack.com` and its subscribers still exist and are labelled *(retired)* wherever they surface; Krish owns the Substack-side migration.
...
### 2026-08-11: the portfolio refocuses on Mindmaker Live, and MYMU becomes the CTRL door

Krish's directive: *"Ctrl has merged with Make Your Mind Up, now forming part of the lead magnet to the Ctrl app hosted at makeyourmindup.ai. All content will be housed under MindMaker Live, which is the content arm of MindMaker, the advisory."* This reverses the 2026-08-06 entries below, which made MYMU the content brand.

**The model now.** ONE media venture, `mindmaker_live`, with exactly two formats: **Paid** (the investigation, carrying the retired Techonomic register) and **Built** (carrying the builder economy thesis). `Make Your Mind Up` is a PRODUCT surface, the lead magnet into CTRL at makeyourmindup.ai, and `ctrl.themindmaker.ai` already 308s there. Builder Economy is fully retired. Signal & Noise is demoted from venture to distribution channel, feed and subscribers deliberately untouched with no public change made. The MYMU weekly format is retired because that name is now the CTRL door.

**Adtech and martech are deliberately NOT scrubbed** (Krish: history, content and networks are kept, it is simply not the specialty). All 1,405 contacts, the 522 mindmaker-laned opportunities and the publisher/adtech keyword sets stay fully live and scoreable; only the positioning changed.
