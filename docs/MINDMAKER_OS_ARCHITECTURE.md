...
| **O-3** | One person running what traditionally takes 15-30 | Active workflows × success rate × outputs landed | ~91 active workflows. Vera scores fleet health weekly. |
...
| N8N Cloud | ~100 workflows (~91 active, ~7,651 scheduled execs/mo, 10k/mo cap) running on cron/webhook — orchestrator, agent jobs, integrations | `krishraja10101.app.n8n.cloud` |
...
### 3.4 N8N workflow inventory (100 workflows, 91 active, 9 inactive)

Live inventory (reconciled against the runtime 2026-07-01), grouped by name prefix. Active counts reflect the 2026-07-01 changes (2 broken workflows disabled, Kai mapper re-enabled — see §3.4.1 / changelog):
...
| **Active total** | **91** | |
| **Inactive / archived** | **9** | Workflow Optimizer; ZZ ARCHIVED Agatha Visibility Deep Enrich (dup); Nell Guest Speaker Briefing (archived dup) + Guest Pitch Enrich (archived); Nova Visibility Backfill Tick; System HARO Ingestion; "AI Agent workflow" (legacy); + Maya Churn→Exit & Sonnet Task Lever Rater (disabled 2026-07-01) |
...
Because ~99% of executions are cron/schedule-driven (webhooks measured at ~1%), the monthly total is deterministic from trigger config. Current steady state after the 2026-07-01 reconciliation: **~7,651 scheduled execs/mo** (see the per-workflow budget in the audit workspace). Two layers keep it there:
...
**Monitor migration (the "permanent fix").** The zero-AI `Critical Infrastructure Monitor` (POST Supabase RPC `audit_critical_infra` → Telegram on failures) is ported to a free VPS cron (`critical-infra-monitor.py`, `*/5`), giving real 5-minute coverage at zero n8n cost. **Deployed 2026-07-01** — both scripts live under `/root/.openclaw/workspace/scripts/` and wired into the root crontab (governor hourly, monitor `*/5`); the governor's cumulative counter is seeded. The n8n `Critical Infrastructure Monitor` (3h) is kept **active as a redundant backup** — retiring it (to save ~240/mo and avoid duplicate alerts) is a one-toggle follow-up.
...
N8N workflows carry their own `cron` / `schedule` nodes. The ~100 workflows (~91 active) together fire ~7,651 scheduled executions/month (~99% cron-driven, ~1% webhook). See `workflow_runs` for the live cadence and §3.4.1 for execution-budget governance; Kai's Dependency Mapper rolls it up.
...
- **Schedules restored (budget-aware).** `System | Audience Pipeline` sync 3h → **15m** (node renamed "Sync every 15m"); `Nova | Visibility Sweeper` retry sub-trigger 12h → **6h** (weekly Mon 11:00 sweep unchanged). `Critical Infrastructure Monitor` kept at **3h** in n8n (its "restore to 5m" is delivered off-budget by the VPS port instead). Net steady state **~7,651 scheduled execs/mo** (~99% cron), under the 8k target and 10k cap.
