...
>
> **Last verified against live state.** 2026-05-25, post audit closure PRs #67 → #70 (action-surface buildout + repaired Agatha pipeline + new Cleo Email Draft workflow + N8N workflow snapshots checked into git).
...
2. **Supabase is the single source of truth.** Every piece of state — agent identity, sprint plans, tasks, leads, guests, customers, bets, standards, audit log, completeness contracts, silent failures, email drafts — lives in one Postgres database (~65 tables). Local JSON for state is banned.
