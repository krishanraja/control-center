...
3. **Brief assemble, Fri 18:00 UTC** (`/api/briefs/assemble`): drafted-first in the MYMU shape (Headlines / What this actually means / Perspectives), grounded in the shifts register + krish-voice + content corpus → `weekly_briefs` `status='ready'` + a `brief_review` decision.
