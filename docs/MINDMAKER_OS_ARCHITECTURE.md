...
>
> **2026-06-01 (Phase 6, PR #111).** Objective altitude made editable + voice-narratable (Marcus recalibrates the milestones beneath); weekly slate replaces the starved accepted-only pool (eligibility 2 → 21); `milestones` gained `owner_split`/`is_accomplishment`/`auto_executable_pct` and `get_objective_tree` now returns them + `goal_agent_contributions`; commit ladders committed milestones into agent-assigned tasks; Builder Economy re-leveled into the `obj:builder-economy:podcast-launch` objective. See §5.7.
...
### 5.7 The objective altitude — review, slate, and the OS-vs-you split (Phase 6, shipped 2026-06-01, PR #111)

**Why.** The weekly takeover made Krish micro-approve milestones instead of reacting to the objectives themselves, and the commit picker was fed only by `accepted` milestones (2 existed), so it was starved. Phase 6 moves the top step to the OBJECTIVE altitude and unstarves the week.

**Objective altitude is editable.** `goals` rows are no longer read-only. `PATCH /api/objectives/:id` edits title, priority, `definition_of_done`, `why_now`, `target_horizon`, `primary_kpi`, `secondary_kpi`, `status`, `is_auto`, or reorders priority (`action:'reorder'`). A title reshape writes an objective-altitude `feedback_queue` row (`marcus_objective_amended`, or `marcus_objective_releveled` for a promote/demote). `FocusRitual`'s `PortfolioStep` is rebuilt around editable `ObjectiveReviewCard`s (reorder, inline edit, narrate, OS-run toggle); milestone shaping is demoted to a per-card disclosure.

**Voice → Marcus recalibration.** `POST /api/objectives/voice` (audio; reuses the shared Whisper `transcribeRequest`) transcribes Krish's narration, then Marcus (Sonnet 4.6) returns a STRUCTURED, confirmable change-set `{ summary, reprioritize, edits, relevel, recalibrate_goal_ids }` grounded in the live portfolio. Nothing applies until Krish confirms in `ObjectiveProposalReview`; apply then PATCHes the objectives and fires `propose-milestones` with `mode:'recalibrate'` + `krish_context` so the milestones beneath a reshaped objective regenerate. The proposer (`uL8DLpHbT11eqBAW`) now reads `mode`/`krish_context`, drops stale `proposed` milestones in recalibrate mode, and emits the three new milestone fields below. (That workflow has `availableInMCP=false`; the canonical edit is in `scripts/n8n/krish-objective-milestone-proposer.workflow.json` and must be re-imported to n8n Cloud.)

**Weekly slate.** `GET /api/weekly-focus/slate` replaces the starved pool: every accomplishment-altitude milestone (`is_accomplishment=true`, status in proposed/accepted/active) across ALL active objectives, breadth-first by objective priority, cap 12, each carrying its ladder-up objective + the OS-vs-you split. `WeeklyStep` consumes it and adds a write/speak-your-own entry (creates a `krish_authored` milestone). Eligibility went 2 → 21 after the Phase 6 re-level.

**Ladder-down.** `POST /api/weekly-focus/commit`, after writing `weekly_focus` + the bridge, spawns one `tasks` row per contributing agent for each committed milestone (deterministic, idempotent on `(milestone_id, agent)` in the `objective-ladder` workstream), so the daily picker's `serves_milestone` join has candidates immediately. Implemented **inline with the service-role client** (no n8n workflow, no inlined secret — a deliberate departure from "n8n for everything" for a pure DB fan-out). It also captures weekly-altitude taste (`marcus_weekly_slate_override`) from hand-written or dismissed moves.

**OS-vs-you split.** New `milestones` columns `owner_split jsonb` (`{ os:[{agent,does}], krish:[…] }`), `is_accomplishment boolean default true`, `auto_executable_pct int`. `get_objective_tree` now returns these plus the objective's `goal_agent_contributions` (populated since Phase 1e, surfaced only now). The `OwnerSplit` component renders "OS drives / You own" on the ritual, the `ObjectivesPanel` deep-work block + expanded row, and every slate item (falling back to the contributing agents when a milestone has no explicit `owner_split` yet).

**Data re-level (live).** "Build a repeatable Builder Economy guest pipeline" was milestone-altitude; promoted `obj:builder-economy:podcast-launch` ("Launch Builder Economy podcast with guests in the pipeline") as the P2 objective, re-homed the pipeline milestones under it, backfilled `primary_kpi`/`definition_of_done`/`why_now` on it and on `obj:mindmaker:lead-nurture`, and set `is_accomplishment=false` on "Consolidate and score all existing leads" so it leaves the slate. Migrations `scripts/migrations/2026-06-01-objective-altitude-review.sql` + `2026-06-01-relevel-builder-economy-and-backfill.sql`.

**New feedback reason codes** `marcus_objective_releveled`, `marcus_weekly_slate_override`; new source_table `weekly_slate`. All three altitudes (daily / milestone / objective) stay distinct per §8.7.0.
...
## 22. Audit reconciliation — 2026-05-26
...
### 22.3 P1 deeper fixes shipped in second batch (5 items)
