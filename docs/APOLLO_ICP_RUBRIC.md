...
2. **"AI" as a keyword recruits the supply side.** It surfaced people who *work
   ...
   (`mindmake`, `mm_ctrl_buyer`) now **drop the AI keyword and exclude vendor
...
3. **`mindmake_buyer` is hard to target positively.** The only genuine buyer the
...
|---|---|---|---|---|
| **Mindmake buyer** | `mindmake_buyer` | `mindmake` | AI consulting sprint buyers | **O-2** ($20K/mo consulting) |
| **Fractional network** *(new)* | `fractional_network` | `mindmake` | Fractional execs / independent advisors / boutique AI consultancies — referral, co-delivery, and buyers | O-2 + O-3 |
| **Signal & Noise guest** | `signal_noise_guest` | `signal_noise` | AI-in-media voices for the podcast | content / audience |
| **Built guest** | `built_guest` | `publication` | AI builders / indie hackers / technical founders | content / audience |
| **mm-ctrl buyer** *(outside-box)* | `mm_ctrl_buyer` | `mindmake` | Leaders who'd buy the decision-clarity product (CTRL) | builder-product growth |
| **Ecosystem partner** *(outside-box)* | `ecosystem_partner` | `mindmake` / `publication` | Accelerators, communities, agencies, VC platform leads who channel buyers + guests | O-2 + audience compounding |
...
|---|---|---|---|---|
| `mindmake_buyer` | Founder, CEO, COO, President, CMO, CTO, Chief of Staff, VP/Head (Ops/Marketing/Product/Strategy) | owner, founder, c_suite, partner, vp, head | `11,50` `51,200` `201,500` `501,1000` `1001,5000` | SaaS, software, professional services, media, fintech; kw "AI", "transformation", "digital" |
...
**`mindmake_buyer`** (matches seed) — role_fit **0.30**, intent_signals **0.25**,
...
```jsonc
{
  "icp_scores": {                 // → leads.icp_scores (jsonb)
    "mindmake": 0-100,           // keyed by lane; registry slugs use the slug,
  ...
  "best_lane": "mindmake_buyer",
  "best_score": 0-100,
  "tier": "A" | "B" | "C",        // → leads.tier
  "icp_score": 0-100,             // → leads.icp_score (int, = best_score)
  "primary_venture": "mindmake", // → leads.primary_venture (FK; registry slug)
  "tags": ["mindmake_buyer"],    // → leads.tags (every lane ≥ tier-C threshold)
