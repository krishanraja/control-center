...
- decision (Krish, 2026-09-07): the docs steward is adopted across eight repos. `NOW.md` at the root is the one file agents are promised is current; this log is where superseded material goes instead of being deleted. Procedure: `docs/steward/RUNBOOK.md`. The three weekly "Documentation Refresh" Routines that wrote to unmerged `claude/*` branches are disabled, not deleted.
