...
**3. Evening shutdown.** `src/components/pilot/EveningShutdown.tsx`. Tomorrow's 3, chosen tonight: the ONE (required, concreteness-checked, what red mode reads tomorrow), a second and a third (optional), each able to name the weekly objective it serves, plus what shipped today and a link. Auto-prompts after 5pm on first interaction, once per day. Dismissing it (X, "Not now", Escape, backdrop) closes it **until tomorrow on every device**: it writes an evening row with `skipped = true` and a civil-date flag in localStorage (`src/lib/dayFlag.ts`). The first version kept the flag in sessionStorage, which is per tab, so every new tab and PWA relaunch brought the prompt back.

> **One writer for tomorrow (2026-09-08, ADR-018).** The evening `POST /api/pilot/checkin` writes the `pilot_checkins` row AND tomorrow's `daily_focus` row through `api/_dailyFocus.ts` (`upsertSlots`). Red mode keeps reading `tomorrow_one`; Home's Today list reads `daily_focus`; both come from the same save, so the two daily commitments that used to live on two clocks are one object. The morning gate's set screen shows "Last night you chose: …" and the header line carries what he said shipped.
...
`GET /api/pilot/checkin` returns today's morning row, the most recent evening row (one that carries a ONE; a skipped evening does not count), and whether tonight's shutdown is done (a skipped evening does count, so the prompt stays away). `POST` writes a morning or evening row, idempotent per civil day; an evening row also upserts tomorrow's `daily_focus` slots; `{ kind: 'evening', skipped: true }` closes the prompt for the day. `PATCH` records the red mode override.
