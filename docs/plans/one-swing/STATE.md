...
- Env added to control-center (production and preview): `GITHUB_REPOS` (the fifteen active repos), `GITHUB_AUTHOR=krishanraja` (no longer read since 2026-09-07: every commit in every listed repo counts), `UNASKED_HOURS_PER_COMMIT=0.5`, `VITE_BRIDGES_LANE_ENABLED=false`. (That variable is retired 2026-09-15: it outlived the 2026-09-07 un-parking and kept the Hunt lane invisible for eight days. src/lib/bridgesLane.ts no longer reads it.) `GITHUB_TOKEN` already existed and is reused; the first Saturday run (04:00 UTC) proves whether it can read commits. `PARTNER_EMAIL` unset until a partner exists.
