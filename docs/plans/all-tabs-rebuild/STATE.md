...
| Growth | DONE 2026-09-10, hero fixed 2026-09-11 | The last tab to go through this rebuild. Renders through the shared `DoThisNextHero`; five sections reordered to run in the order one causes the next (Review → To do → What's moving → Where they are → Spend limits) instead of the old map-first order; lands on "To do" rather than the reference map. Verified at 390x844 in both themes. 2026-09-11: the hero's action now works when its own section is already open (it used to no-op), and the phone drops the purpose and section-purpose lines the hero already stands in for. See `docs/PRODUCT.md`, Tab: Growth. |
