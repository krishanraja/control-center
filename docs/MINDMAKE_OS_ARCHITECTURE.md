...
| **`/api/aeo/subjects`** | GET / POST / PATCH | The AEO subject registry: list, add a venture, prospect or aspiration, edit or retire (active=false) |
| **`/api/aeo/context`** | GET (Bearer `AEO_ENGINE_SECRET`) | Everything the AEO engine needs for one week: active subjects with domains and competitors, geo touchpoints, striking-distance keywords (through `LANE_SLUG`), the last 28 days of probes, the linked Room target and the face ICP for a prospect, and the prior week's digest and queries |
| **`/api/aeo/ingest`** | POST (Bearer `AEO_ENGINE_SECRET`) | One AeoPacket per subject per week (`docs/AEO-PACKET.schema.json`): validated, idempotent on run_id, replaces the week on a new run_id, writes probes, queries and the digest, pushes up to three recommendations into `content_ideas` as `aeo_signal` under the build-signal governor, closes the Run-now ledger; every run recorded as `aeo_ingest` in `content_engine_runs` |
| **`/api/aeo/run`** | GET / POST (cookie), PATCH (bearer) | Run now: queue a row in `aeo_commands`, fire `repository_dispatch` on `AEO_REPO`, say honestly whether it was sent; the engine reports running or failed |
| **`/api/aeo/digest`** | PATCH | Dismiss or restore one recommendation on a digest, the one write the tab makes to the engine's row |
...
**Strategic intent.** Before this, "what should I do?" was answered by five overlapping Home surfaces, and the daily `top_three` always elevated atomic tasks, so multi-week objectives never won daily airtime. The Focus System makes one spine run from the canon down to the work on every tab - and since the 2026-08-20 recompose that spine is exactly three layers: **OS GOAL → WEEKLY OBJECTIVE (≤3, each serving an OS goal) → TODAY'S 3** → every tab's list. One sitting a week sets the weekly objectives (directly - the milestone/slate machinery is retired); one commitment a day picks the 3; everything else reorganizes behind those two choices. The whole canon (all three layers) rides `api/_goals.ts` into every reasoning path, and since 2026-09-08 (ADR-018) that block also carries today's frame (the morning intent, venture and mode, last night's ONE, what shipped) and the last four closed weeks with their outcomes. **Cadence (2026-09-08):** a weekly objective carries `goals.week_start` (the operator's Monday; a weekend entry targets the coming Monday). A Saturday 05:00 UTC cron (`/api/goals/week-close`) closes the week: done stays done, still-active becomes `missed`, nothing is deleted, the `goals` table is the archive and `GET /api/goals/history` reads it by week. Monday's ritual offers last week's outcomes with one-tap Carry (a fresh row linked by `carried_from`). Today's 3 are manual first: the evening shutdown writes tomorrow's three slots through `api/_dailyFocus.ts`, Home's Today slots are editable in place (`POST /api/daily-focus/slot`), and the machine's suggestions sit behind a disclosure in the ritual. The weekly composer is the ritual alone; the ladder composes only the OS goal at cold start. It is engineered around behavioral psychology (fresh-start effect, peak-end, implementation intentions, commitment-and-consistency, goal-gradient, Zeigarnik, Hick's law, loss aversion) and information retention.
...
- **The upstream pile (was triage mode):** the swipe deck over seeds and research now lives inside the mobile Queue (`MobileDecisionDeck` + `useContentTriage`). Left / Not for me = drop with a reason (undoable), Write this = send to drafts (`review` / `approved` open the Composer; the two human gates are never auto-crossed). Commits are optimistic via a session `committed`-id set. The standalone `TriageDeck` was deleted 2026-09-07.
...
- **Transform axes** - one-click tone (punchier / contrarian / warmer / formal), length (short LinkedIn / mid Built / full essay), and zoom-into-sharpest-angle. Preview-then-accept: the row's `body` is not mutated until Krish accepts; a non-destructive history lands in `meta.revisions[]`.
...
- **Channel variants** - retired 2026-09-07 with the `/transform` route; channel cuts in the Composer's Produce stage (`/channel-cut`) are the one path to a per-channel version.
...
**Mobile is quick decisions and quick edits, not deep work (`ContentV2Tab` Queue + the Composer's `narrow` body).** Since 2026-09-07 the phone leads with the finite decision deck: the weekly brief, video reviews, the typed rulings, then the upstream idea pile one card at a time (Write this / Open / Not for me with a reason). The rooms sit beside it as segments and lead with the one Next action. Opening a piece on mobile renders the draft in read mode (no keyboard) with a one-tap magic row that previews inline, Cleo / Materials / Research as secondary sheets, and a big sticky Save Draft. The desk is where a piece is developed: lanes show the Next card, ideas ready to shape, the state board and the folded context; the Composer rail is three stages (Draft / Strengthen / Produce) and opens on the one the piece is at. The retired `MobileContent` and `DesktopContent` surfaces, and the mode-switched `TriageDeck`, are deleted; `docs/CONTENT-ENGINE-PARITY-LEDGER.md` names where each of their features went.
...
**`/api/content-ideas/:id/*` routes.** `revise` (transform axes + iterate, voice + materials-grounded, preview-only), `challenge` (tiered enrich → `meta.challenges[]`), `score` (Five Standards → `meta.standards` + `quality_score`; optional `model` param), `dive-deeper` (scoped Perplexity → `meta.deep_dives[]`), **`materials`** (GET/POST/DELETE the corpus on `meta.materials[]`), **`chat`** (Cleo writing partner, multi-turn → `meta.cleo_chat`), **`save-draft`** (sanitize + save body, attach materials, fire the content-factory webhook, → `review`; the Composer's end CTA. `push-to-cleo` and `transform` were deleted 2026-09-07: nothing called either). Shared helpers in `api/_content.ts` (Claude single-shot `callClaude` + multi-turn `callClaudeMessages`, voice / corpus / materials grounding, `sanitizeVoice`, CORS). The content-factory webhook URL is server-side + rotatable (`N8N_CONTENT_FACTORY_WEBHOOK_URL`).
...
**Lane → factory channel.** Two taxonomies overlap but differ: ventures generate variants (`content_lane_*` voice configs: publication paid/built, signal_noise, builder_economy; `builder_economy_ig` is RETIRED as a lane, Instagram is a channel, see §0a), while the factory polishes into `signal_noise | makeyourmindup | linkedin | builder_economy | vertical_video | dynamic`. `src/lib/contentEngine.ts` holds the map plus the transform-axis presets and the Five Standards definitions. Since the 2026-08-06 the Paid format retirement (§11.3) the stored lane values `techonomic` and `publication` are legacy: `normalizeLane()` reads both as Mindmake and every server mapper folds them onto the `makeyourmindup` factory channel, so nothing throws on an old row. `publication` became a legacy value on 2026-08-06 when the channel was renamed to `makeyourmindup` (§20).

**No feature flag.** `VITE_CONTENT_ENGINE_ENABLED` was removed 2026-09-07; the engine is always on and `scripts/check-env-example.mts` fails the build for any unlisted variable. Server keys: `N8N_CONTENT_FACTORY_WEBHOOK_URL`, `PERPLEXITY_API_KEY`, `ANTHROPIC_API_KEY`, optional `NEWSAPI_KEY` and `APIFY_TOKEN` (real Reddit/LinkedIn community scraping in Challenge; degrades to the Perplexity forum pass without it; `APIFY_REDDIT_ACTOR` / `APIFY_LINKEDIN_ACTOR` override the actors). All set in prod 2026-06-11; engine live.
...
**Rooms** (the Content tab, the only surface since 2026-09-07; originally four rooms, now Built / Paid / Library with the obligations in a strip):

**Run ledger and runner watch (2026-09-07).** Every content cron is wrapped in `withContentRun` (`api/_runs.ts`) and writes one `content_engine_runs` row: ok, skipped with the reason, or failed. `feed/ingest` fails its run when the pool is unconfigured instead of returning a 200 nobody reads. `src/lib/contentEngineSchedule.ts` names every job and its allowance; `ObligationStrip` reads the ledger and says, in the same strip as the brief and the video reviews, when a job has not succeeded in time. `api/video-studio/runner/watch.ts` (daily 06:30 UTC) writes a failed run when the Windows runner has been silent for a day with commands, reviews or approved briefs waiting. `scripts/check-content-engine-schedule.mts` holds the table and `vercel.json` to the same set. The OS stays pull-only: nothing is pushed, the tab says it.

**Video Engine and the Studio boundary.** The video and carousel engine is `krishanraja/content-engine`, which since 2026-09-08 (ADR-019) also serves the control plane: `apps/control-plane` holds the editorial routes, the Composer routes, the fourteen content crons, the thirteen `video_studio_*` tables' routes, the portable MCP gateway and the `mindmake_studio_*` learning spine, deployed as its own Vercel project. Control Center reaches them through rewrites, so every URL is unchanged, and keeps the Content tab, the Composer, the deck and the reviewer. Editorial judgement still lives in the engine's routes, not in the renderer: the studio radar is a feed importer, and production starts from an approved `ProductionBriefV1` the runner claims. Reviews reach Krish through the mobile deck and the desktop obligation strip; learning proposals reach him in the Library. Activation state for the runner and the consumer OAuth connector is the ledger in `docs/plans/video-engine/STATE.md`.
...
| Mindmake Strategy | `1W3maI4PQvy21iP8FrMJr4IO5QWbS6Q3U` | Sprint outputs, AI consulting proposals |
...
| **Signal & Noise** | (podcast) | A **distribution CHANNEL**, not a venture, since 2026-08-11. AI in media; co-hosted with Rio Longacre + Brett House. Carries Mindmake's publication material. Feed, GUID and subscribers deliberately untouched; no public repositioning made |
...
| **CTRL** | `mindmaker_llc` | `ctrl.mindmake.co/.well-known/product.json` | LIVE (`WAREHOUSE_INGEST_URL` + secret) |
...
The autonomous customer-acquisition layer. Doctrine is canonical in `acquisition-os-playbook.md` (v1.1, OneDrive: `Documents/0 Ventures/Mindmake/GTM playbook for autonomous agents/`); this section records only what is wired into the OS.
...
**The ask (Krish).** The white-glove AEO research agent from a LinkedIn post (the last 7 days of calls to 5 to 10 themes, the topics where competitors outrank us in LLM search, 20 to 40 scored queries, a weekly digest with article recommendations, a watch list and the biggest gap), rebuilt as his own "for all my ventures, or companies I want to aspire to, companies I want to sell to ... so many clues to figure out how to be in front of them in AEO", automated and autonomous, always oriented to insight and action, with the machine in `krishanraja/AEO-Engine` and the UI in Control Center, reimagined rather than bolted on.

**What shipped in this repo** (spec `docs/AEO-ENGINE.md`, contract `docs/AEO-PACKET.schema.json`).
- **A subject registry** (`growth_aeo_subjects`): a venture, a prospect (optionally tied to a Room target) or an aspiration, with domains, competitors, seed topics and a never-say list. The engine holds no venture list; the five ventures are seeded and Krish adds the rest on the tab.
- **One probe table, extended.** `growth_geo_probes` gains `subject_id`, `subject_kind`, `run_id` and `query_id`; its product_slug CHECK now applies to venture rows only, so a prospect's or a benchmark's answers sit beside the ventures' without a sibling table. Historic rows are joined to their venture subject as the baseline.
- **`growth_aeo_queries` and `growth_aeo_digests`**, the scored corpus and the weekly read, and `aeo_commands` for Run now.
- **Four routes under `api/aeo/`**: `context` (what the engine reads), `ingest` (where the packet lands, idempotent on run_id, run-ledgered as `aeo_ingest`), `run` (queue, dispatch, honest `dispatched`), `subjects` and `digest`.
- **Recommendations join the content spine** as `source_type='aeo_signal'` (three per subject-week, the build-signal governor at twelve open rows, a live `source_ref` index), judged by both editorial lenses and routed with `meta.aeo` on the child.
- **The Sunday council reads the digest**: `Evidence.aeo` carries the strongest signal, the call themes, the gap and the top recommendations; a missing digest is an unknown, never no demand; the measured line gains an AEO segment.
- **Guards moved with the code**: `check-content-engine-schedule` learns an external job, `check-build-signals` pins the three-element radar literal.

**What is deliberately not done yet.** The Monday `api/growth/geo-probe` cron stays until the engine has two green Sundays. Legibility is not a subject until the slug debt is normalised. The Signals surface is rebuilt after a rendered mock is approved (krish-design), not before.

**Honesty recorded.** There is no prompt-volume corpus here; demand is a labelled proxy (engine answerability, call evidence, Google volume where a keyword matches). Call material reaches the tab as paraphrases with an opaque call reference; the ingest refuses a packet carrying an email or a handle.
...
**Phase 2 shipped the same day (Krish: "do both").** The Inspiration Sweep (`D4W5TF1sP9lE828c`) gained a `Load Creators` HTTP node (content_creators, active only, same supabaseApi credential and settings as `Load Recent Angles`) wired `Load Recent Angles -> Load Creators -> Build Extraction Prompt`, and the extraction prompt now carries a `## CREATORS KRISH RATES` section: a screenshot or doc by a curated creator is prized inspiration, extract the transferable move, write Krish's own take, a paraphrase is a rejection, fill `poster_name` and `poster_handle`. Applied through the n8n management API against the live definition (never the snapshot), draft diffed byte-for-byte against the validated code before publish, published as version `2b03c6d8`, verified by manual execution 40437 (success, `creators_count` 10, the assembled system prompt names Andreas Horn and Alex Lieberman, zero new inputs so an idempotent `Audit Log Zero` exit with no writes). The repo snapshot `scripts/n8n/cleo-inspiration-sweep.workflow.json` carries the identical change. Live-data follow-ups also done: migration applied via the Supabase connector (ledger name `content_creators_registry`), `apify_actor_registry` row for `linkedin_profile_posts` inserted, readback confirmed 10 creators, 5 scrapeable.
...
**The portfolio allowlist**, read live from the krishraja.com portfolio section: Mindmake (advisory), CTRL (product), Content (the publication), Signal & Noise (podcast, distribution plus guest acquisition), Full Time, Pulse and Circle (build experiments acquiring test customers, never products for sale). Nothing outside that list is a live venture. Fractionl is not a brand; the display names are Circle and Pulse.

**What changed.**
- `Mindmaker` to `Mindmake` and `Mindmaker OS` to `mind/make OS` across the repo, the skills, the runbooks, the live content corpus and all 14 agent briefs.
- venture_registry: new live rows `mindmake` and `publication`; the old rows kept and marked inactive so historical rows still resolve. `signal_noise` reactivated. FK children and every venture/lane/product column migrated. `customer_product` enum values renamed in place.
- The publication runs exactly two channels, **The Money of AI** and **Built with AI**. `venture_formats` and `content_ideas.lane_slot` moved to `money_of_ai` / `built_with_ai`, with canon mandates written in. The live `system_config.content_corpus` headings were renamed to match, and slicing was verified against the live corpus for all five channels.
- The offer ladder is retired. Two doors, one privately priced thirty-day paid proof.

**Deliberately NOT changed, because it is infrastructure and not a name.** `krish@themindmaker.ai` (the only published contact address; the mailbox migration has not been done), `live.themindmaker.ai`, `mindmakerlive.substack.com`, `maven.com/mindmaker`, the Vercel project `control-center-mindmaker`, the webhook path `/webhook/mindmaker-orchestrator`, `Mindmaker LLC` and the `mindmaker_llc` Stripe key. A blind rename broke several of these during this pass and each was restored and re-verified. **Adtech and martech history, contacts and network stay fully live.**

**Acquisition doctrine.** No cold email. The only two sales motions are warm intros and published thinking. Cold contact is legitimate for exactly one purpose, booking podcast guests. The OS optimises for, in order: pitch meetings for Mindmake, podcast guests confirmed, content published. Agent autonomy was raised for guest booking, warm-intro surfacing and meeting prep; the authority boundary is unchanged and nothing is sent or published without per-asset approval.

**Two live defects found and fixed.**
1. `Felix | Opportunity Pipeline Tracker` was **active and had run 40 times that morning** for an agent retired on 2026-07-10, doing Instantly cold-outbound work against current doctrine. Krish's July ruling to unpublish it had been reverted. Unpublished again.
2. `sync-briefs-to-skills.sh` runs Google Doc to SKILL.md to Supabase, and the Docs were far thinner than the Supabase briefs (cleo 7.5k against 28.6k, zara 2.5k against 21.4k). An unguarded run **deletes the richer brief and resurrects retired brands**, which is why the 2026-08-06 purge kept coming back. A **revert guard** was added (refuses when the Doc would drop the canon block or shrink the brief by more than 20 percent, override `FORCE_BRIEF_SYNC=1`), and the five doc-backed Docs were then rewritten from Supabase so the sync direction is safe again. Verified: the zara sync now passes cleanly.

**The Windows `mindmake-os` skill was restored to a thin router.** It had been overwritten with the full ~338 KB body at some earlier point, contradicting the documented design. It is now a ~5 KB router that names the authorities and maps the sections, and carries no architecture claims that can go stale. It is not one of the synced surfaces.


### 2026-08-24: the learning loop finishes a lap, and a fix that was never published

The 2026-08-19 work made Vera's feedback loop run end to end for the first time. Watching the first real lap showed it did not finish one: it wrote corrections but never finished consuming what it read, so the same clusters would have returned every Sunday forever.

- **Provenance stops coming from the model.** `consumed_feedback_ids` was whatever Sonnet echoed back. Asked to reproduce UUIDs for clusters covering 191 rows inside a 4000-token budget, it returned a 50-id sample and dropped 2 of 10 clusters outright. Ids, agent and reason code now join from the clusterer's own output on `cluster_key`; a `cluster_key` the clusterer never emitted is dropped rather than written as a correction with no traceable evidence. The model is no longer asked for ids at all, which also stops the prompt shipping 191 UUIDs it cannot use.
- **And the fan-out stops reading one item.** `Fan out feedback ids` used `$input.first().json` while `Write Corrections` runs once per correction, so with 8 corrections it fanned out only the first. Between the two bugs, 10 of 187 eligible rows were ever marked consumed. Data repaired alongside the code: the 8 pending corrections were backfilled to true membership (187 ids) and those rows consumed. A verification run then found the 2 clusters the model had dropped and consumed all 4 of their rows. The next scheduled run sees no duplicates.
- **A new class of silent failure: the unpublished draft.** n8n Cloud keeps a draft and a published version, and an edit made through the MCP `update_workflow` tool writes the **draft**. A manual test run executes the draft and passes while the cron keeps running the old published version, so a fix looks shipped and is not. Vera's provenance fix passed a manual run while Sunday would still have executed the broken code. Nothing in the OS could see this: the workflow does not error, its heartbeat is written, its counts look healthy. `fleet-reconcile` now compares `versionId` against `activeVersionId` (both already on the workflows list it fetches, so no extra API calls) and raises its own tier-2 `silent_failure`; `workflow_health.unpublished_draft` carries it. The audit row also records how many workflows reported both ids, because zero drafts and a build that does not report versions are different facts and only one of them is good news.
- **The repo copies stopped lying.** The checked-in Vera JSON declared `credentials.httpHeaderAuth` with no `authentication` parameter while live used `predefinedCredentialType`/`supabaseApi`. A `PUT` replaces the `nodes` array wholesale, so syncing it detached every credential and broke the workflow until live was restored from a pre-PUT snapshot. `scripts/n8n/README.md` had asserted the opposite on both counts (that `audit.mjs` ignores credentials, and that n8n preserves them on sync); both claims are corrected, and the four traps that have actually caused outages are written down.
- **One earlier claim in this document was false.** The 2026-08-19 entry said two classifier bugs were "caught by unit tests". No test existed; `api/` had no unit-test runner at all. `scripts/check-fleet-classifier.mts` now covers both orderings (quota before credential, scheduled-and-never-ran as dead) and runs in CI, verified to fail when either regression is reintroduced.

### 2026-08-19 (later): the OS stops grading its own homework, the backlog starts ageing out, and the feedback loop closes for the first time

Krish asked three things: stop the news randomly drying up, archive what he does not action so he only ever sees fresh material, and make his standards actually compound. The design for all three already existed. Two of the three had never once run.

**The structural fault, common to all three.** Every health signal in this OS is self-reported by the workflow that is supposed to be healthy, and heartbeats are written at the END of a run. A workflow that dies at node 3 writes nothing, and nothing is indistinguishable from "not scheduled today". So the failure mode that actually happens is exactly the one the monitoring cannot see. `credential_health` held 20 rows, all `healthy`, last verified 2026-05-19, while three workflows failed on credential errors every day for three months. The sensor was unplugged and stuck on green.

- **New external observer.** `api/health/fleet-reconcile.ts` (Vercel cron, every 6h) reads the **n8n executions API** and writes `workflow_health` + `fleet_failures`. Deliberately a Vercel cron and not an n8n workflow: an n8n workflow monitoring n8n shares the blind spot it exists to close. There is already a `Truth Reconciler` workflow with a `Fetch Live Workflows` node that was meant to do this; it has failed 2/2 on an expression error, so the fix itself was broken. Failures are grouped by cause (`credential | quota | logic | network`), so a credential outage is one story rather than six unrelated alerts. First run over live data: **10 dead, 6 degraded, 1 failing** out of 121 workflows, 108 failed runs traced to three broken credentials and 26 to two blown quotas.
- **Two classifier bugs caught before deploy.** n8n wraps almost every non-2xx in "Forbidden - perhaps check your credentials?", and puts the actionable cause in `error.description`, not `error.message`. Classifying on the message alone made Zara's "Monthly usage hard limit exceeded" and the Orchestrator's Google Drive rate limit both read as broken credentials, which would have sent someone rotating perfectly good keys. Quota is now tested before credential, and `description` feeds both the stored message and the classification. These were caught by inspection, not by tests: despite an earlier note here saying otherwise, no test covered them until `scripts/check-fleet-classifier.mts` was added on 2026-08-24 (CI). Separately, `degraded` now alerts at tier 2: Zara errors on only 38% of runs yet has written no `zara_signals` row since 2026-07-24, and silence at 38% is how that stayed invisible.
- **Three workflows repaired.** `HARO Ingestion` had failed **94 of 94** runs against Gmail credential `AFgF8OOuBhYHbiyo`, which no longer exists; rebound to the mailbox the Inspiration Sweep reads successfully every morning. `Vera Feedback Aggregation` was bound to a generic `httpHeaderAuth` credential that sends no `x-api-key`, so it 401'd 2/2. `Maya Customer Acquisition Sweeper` (12/12, "Credentials not found") and the two quota failures are surfaced but left for Krish: one is a plan decision, not a code fix.

**(b) The backlog could not age out, and archiving destroyed the evidence.** The Monday purge exempts drafting/review/approved/published from expiry deletion so started work is never hard-deleted. The gap: an idea promoted out of `seeded` and then abandoned was immortal. 160 live rows, 80 already past `expires_at`; `drafting` 48 of 49 expired (avg 31 days), `review` 9 of 9 expired (avg 70 days, oldest 74).

- **`updated_at` could not detect any of it.** Background re-scoring sweeps touch rows constantly, so those 74-day-old review items reported "touched 8 days ago" and *zero* rows looked idle. New `content_ideas.state_changed_at`, moved by a trigger only on a real state transition, measures decision progress instead of row activity.
- **`archive_stale_content_ideas(days, dry_run)`** buries anything whose state has not moved in 14 days (`system_config.content_staleness_archive_days`), exempting rows linked to a shift, graduated to the library, or published. Run once at 14 days: **53 rows archived**, including all 9 stale `review` items. Everything left past the threshold is shift-linked, i.e. work Krish committed to. Daily at 10:00 UTC via `api/content-ideas/archive-stale.ts`.
- **Archive now means archive, not forget.** `api/shifts/detect.ts` filtered `.is('buried_at', null)`, so hiding a row also deleted it from the trend corpus and "keep reading trends from it" was impossible by construction. Stale-archived rows (`buried_reason` prefixed `stale:`) are re-admitted; rows buried for any other reason stay out, and dedupe burials especially, since their citations already live in their keeper's `meta.recurrences` and would double-count.

**(c) The standard is good. The learning loop had never closed, not once.** Generation-time enforcement is real and working: five active pillars with 6-8 anti-patterns and 3-5 required evidence types each, injected with the voice rules, the CLO-020 bar, temporal class, and `newsletter_source_yield` as a source track record. Feedback-time learning was severed at three separate points.

- **389 feedback rows, `consumed_by_vera = 0` on every single one.** Eight surfaces, 239 on content ideas alone. Nothing Krish has ever told this system has been read. `corrections` holds 2 rows in its whole life, last 2026-05-12, neither from feedback. He stopped giving feedback on 2026-07-01, which was rational.
- **A state-vocabulary mismatch nobody could have seen from either side.** `decisions_waiting` selects corrections `WHERE status='analyzed' AND approval_state='pending'`, and `api/corrections/approve.ts` refuses anything not `pending`. Vera's Parse + Validate wrote `status='open', approval_state='proposed'`. So even with the 401 fixed, every correction it produced would have been invisible and unapprovable forever. Producer corrected to match the two consumers.
- **The backlog will actually yield.** Vera's clusterer finds **13 eligible clusters**, the largest being 143 rejections of Cleo content as `content_thin`. Those become proposed brief edits on the next Sunday run, surfacing in `decisions_waiting` as kind `correction` for approval. Nothing auto-applies.
- **The system was regenerating one idea forever.** The archived pile contained "Three solo builders just shipped what used to take a team of 20" and nine near-identical siblings across six weeks. The daily clustering job had already *detected* this and linked 28 of 49 drafting rows as related; it wrote `related_idea_ids` and did nothing else, so the generator never learned and produced the eleventh. The Inspiration Sweep now loads the last 60 days of angles (buried included, since an angle Krish archived unread is still an angle already produced) and enforces a novelty floor: restating an existing angle with different numbers or companies is `rejection_reason='duplicate_angle'`, not a variation. A subject may be revisited only with a claim the earlier piece did not make.

**Follow-up, 2026-08-20.** Four corrections and one retirement.

- **`N8N_API_KEY` was already set in Vercel and already valid.** I said twice it needed adding. It has been on the project since 2026-04-07, scoped to production, preview and development. It is a *different* key from the one used interactively in that session, and both authenticate: n8n allows multiple API keys. Proven rather than assumed: `POST /api/health/fleet-reconcile` on production returned 121 workflows, 4,456 executions, 12 failing, 18 alerts raised. The reconciler is live and needs nothing.
- **The cron routes had an unauthenticated POST arm, and I shipped two more of them.** `middleware.ts` deliberately exempts `/api/*`, and the house pattern for a cron route is GET-with-`CRON_SECRET` / POST-with-nothing. Verified on production, not inferred: an anonymous `POST /api/content-ideas/archive-stale` returned 200 and executed. New `guardCronRoute` in `api/_auth.ts` requires the secret on GET and either the secret or the edge-gate cookie on POST, with constant-time comparison. Applied to the two routes this work introduced. **Fifteen pre-existing routes still carry the open arm**, worst first: `api/purge/run.ts` (hard-deletes `content_ideas`) and `api/acquisition/governor.ts` (pauses workflows, moves budget). The helper drops into each as a two-line change.
- **`Truth Reconciler` (`rb3QgfBi1NlpTe3t`) is deactivated.** It attempted the same job as `fleet-reconcile` from inside n8n and failed 2/2 on an expression error (`Node 'Fetch Live Workflows' hasn't been executed`). Superseded rather than repaired: a monitor inside the runtime it monitors cannot report its own death, which is the whole reason the replacement lives on Vercel.
- **Maya's Customer Acquisition Sweeper is not a credential problem.** It polls five *external* Supabase projects and none has a credential bound, but three of the five are businesses that no longer exist: Gutted (`hzadscrqmyilbisexvyz`), OnAlert (`zcreubinittdqyoxxwtp`) and Merciless (`cgkcplcamsijghalintq`) are retired or archived per this README and absent from `venture_registry` entirely. Only Fractionl Circle (`ksyuwacuigshvcyptlhe`) and Fractionl Pulse (`dtlcprcpvdomrehbejhw`) are live. Binding all five would resurrect polling of dead products; the fix is to delete the three dead nodes and wire credentials for the two live ones, or retire the workflow. Krish's call. The two `Feedback | Fractionl *` weekly workflows are dead on the same class of cause.
- **The live reconcile surfaced failures the sampled pass had missed**, because error detail is fetched per broken workflow: `Zara | Layer 1 Signal Inbox` (dead, quota), `Agatha | State of Union Weekly` (quota, previously unclassified), `Priya | Weekly Product Rollup` (logic), `Agatha | Weekly Plan Refresh` and both Fractionl feedback workflows (credential).

**Not fixed, surfaced instead.** Zara's news API and the Orchestrator's Google Drive quota are spend decisions. `Truth Reconciler` is now superseded by `fleet-reconcile` and should be retired rather than repaired. Eleven further workflows are failing without a classified cause because error detail is fetched per broken workflow at reconcile time; the first cron run fills them in.

### 2026-08-19: the Inspiration Sweep's Drive lane, audited and given the same spine as Gmail

Krish dropped three LinkedIn screenshots in the Drive inspiration folder and asked whether the content engine actually picks up what he feeds it. Half of it does.

- **The Gmail lane is healthy and was never the problem.** The 2026-08-19 run listed 36 messages in label `AI Newsletters`, found 3 new, extracted 5 raw candidates, and all 5 cleared the pillar bar. 31 seeds in the last 14 days, all newsletter-sourced. The `inspiration_messages` ledger from the 2026-07-27 hardening is doing its job: no message is read twice, `gmail_overflow` is honestly 0.
- **The Drive lane had produced zero seeds in its entire life.** Not zero this week: zero ever. Of 103 `inspiration_sweep` rows, none carries a `drive.google.com` or `docs.google.com` source_url. From roughly 2026-06-25 the 14-day lookback window held nothing at all, so the lane was a daily no-op for eight weeks.
- **It was structurally impossible to notice.** `Heartbeat Done` and `Audit Log Done` reported `gmail_listed` / `gmail_new` / `gmail_overflow` and nothing whatsoever about Drive, while `Build Extraction Prompt` computed `docs_count` / `images_count` / `pdfs_count` and discarded them. There was a `Gmail Zero?` → `Alert Zero Emails` branch and no Drive equivalent. `completeness_contracts` had no row for `D4W5TF1sP9lE828c` at all, so tier 1 never checked the workflow, and "zero rows is a valid output" kept tier 2 quiet. A lane with no counters cannot be missed.
- **The Gemini fallback had never worked, and was about to get expensive.** `Build Gemini Body` read `$input.all()[0].json.anthropic_body`, but its input is the Anthropic node's *error* output (`{ error: ... }`), so `anthropic_body` was always undefined and the fallback posted an empty prompt. It also flattened message content with `JSON.stringify`, which for a multimodal request inlines every base64 image into a text prompt. With `neverError: true` downstream, an Anthropic failure on a 5MB screenshot request would have surfaced as a clean zero-seed "success". Now reads `Build Extraction Prompt` directly and sends images as real `inline_data` parts.
...
Krish's directive: *"Ctrl has merged with Make Your Mind Up, now forming part of the lead magnet to the Ctrl app hosted at makeyourmindup.ai. All content will be housed under Mindmake Live, which is the content arm of Mindmake, the advisory."* This reverses the 2026-08-06 entries below, which made MYMU the content brand.

**The model now.** ONE media venture, `publication`, with exactly two formats: **Paid** (the investigation, carrying the retired Techonomic register) and **Built** (carrying the builder economy thesis). `Make Your Mind Up` is a PRODUCT surface, the lead magnet into CTRL at makeyourmindup.ai, and `ctrl.mindmake.co` already 308s there. Builder Economy is fully retired. Signal & Noise is demoted from venture to distribution channel, feed and subscribers deliberately untouched with no public change made. The MYMU weekly format is retired because that name is now the CTRL door.

**Adtech and martech are deliberately NOT scrubbed** (Krish: history, content and networks are kept, it is simply not the specialty). All 1,405 contacts, the 522 mindmake-laned opportunities and the publisher/adtech keyword sets stay fully live and scoreable; only the positioning changed.

**What was kept on purpose**, because deleting it breaks a historical row rather than finishing the job: every retired lane and channel value still maps FORWARD (`techonomic`, `mymu`, `makeyourmindup`, `builder_economy`, `builder_economy_ig`, `signal_noise`), the factory's Route by Channel still ACCEPTS the old wire values, the Orchestrator keeps its `MYMU` and `TECHONOMIC` aliases, `pillar:builder_economy` stays as a real pillar id, and the two surviving `content_cadence` rows deliberately keep their OLD ids (`cadence:techonomic`, `cadence:builder_economy_ig`) because the hand-run seed uses `on conflict (id) do nothing`, so reusing them is what stops a re-run resurrecting a lane.

**Three structural findings, each of which had been silently reverting work.**
1. **Agent briefs are Google-Doc-driven, not Supabase-driven.** `sync-briefs-to-skills.sh` runs Doc -> SKILL.md -> Supabase and is **manual only** (in no crontab, no openclaw cron job), so the "rendered every ~15 min" note was wrong. Cleo's doc still described the pre-2026-08-06 four-stream structure, which is why that purge kept coming back: it cleaned Supabase while the doc overwrote it. The doc is now fixed at source.
2. **Only six agents are doc-backed** (`cleo, felix, arlo, leo, priya, zara`). Every other agent's `SKILL.md` is a stale artifact the script never touches, and there is no fallback branch, so a failed export leaves bare frontmatter. Those were regenerated from Supabase by hand.
3. **Two n8n visibility workflows query `ventures?select=slug,one_liner,positioning`**, none of which are columns on that table, so they have been erroring rather than returning ventures.

**Corpus.** Headings are now `## 0. Mindmake's publication house register` / `## 1. Paid` / `## 2. Built` / `## 3. Signal & Noise` / `## 4. Maven (free lessons only)`. The Maven playbook was rewritten: the paid ladder it sold is retired, and its job is now feeding CTRL and Mindmake's publication subscriptions. The collision guard got stronger, because the trap recurred a third time: a format name that is also an ordinary English word must have an ANCHORED pattern. "Built" appears inside the real heading "How a piece gets built (the pipeline, per channel)", which sits ABOVE the playbooks, so an unanchored pattern silently hands every Built piece the wrong section. `scripts/check-content-taxonomy.mts` now simulates the real matcher against the live headings and fails on any collision or any live key that matches nothing.

**Landed:** control-center PR #237 (23 files; four hand-maintained lists collapsed onto shared sources so they cannot drift apart again), n8n (factory routing repointed to paid/built plus 9 workflows reconciled in cloud AND git together), Supabase (taxonomy, corpus, per-format voice contracts, agent briefs; all backed up to the `refocus_backup_20260811` schema), and the VPS workspace.
...
**Brands.** mindmake.co is the business (managed advisory + CTRL). makeyourmindup.ai is the content brand at its root; Decide / Extend / Imagine stay in place as lead magnets at their own URLs. Signal & Noise and Builder Economy stay as their own shows on their own feeds, untouched, because Krish still needs guests and a feed rename loses every Apple and Spotify subscriber. MYMU: Built therefore DRAWS FROM the Builder Economy recording rather than replacing it.
...
### 2026-08-06 (later the same day): Mindmake LIVE renamed to MYMU, Techonomic folds in as "MYMU: Teardown"

Krish's directive, superseding the earlier entry below that folded Techonomic into "Mindmake LIVE": *"Techonomic folds in to Makeyourmindup as 'MYMU: Teardown' so I can discern it from other MYMU outputs in the content creator."* One content brand, matching the live property at makeyourmindup.ai. Earlier changelog entries keep their original wording as the historical record; every normative section above is renamed.

- **Slug is `makeyourmindup`, display label is `MYMU`.** Chosen so the content channel and the Vercel project / domain share one name. `FactoryChannel` renamed in `src/lib/contentEngine.ts`; `VentureKey` `publication` became `makeyourmindup` in `api/_finalPass.ts`.
- **`MYMU: Teardown` is a format, not a destination.** Added as a lane slot (`lane='mindmake'`, `slot='investigation'`) so it is selectable and countable separately from the other MYMU outputs, which is exactly what Krish asked for. The slot key stays `investigation` to match the existing `content_cadence` row and the lane config, rather than inventing a second name for the same thing. It routes to the `investigation` corpus playbook and the hard five-lens rubric, so a teardown never drops to the lighter weekly bar.
- **Nothing throws on an old row.** `publication` joins `techonomic` as a legacy value in `normalizeLane`, `LEGACY_LANE_CHANNEL`, `StoredContentLane`, `RETIRED_CHANNELS`, `laneToVenture`, `laneToCorpusChannel` and `laneToChannel`. 187 stored `content_ideas` rows were deliberately NOT rewritten: they resolve forward through those aliases.
- **The `customer_product` enum value `publication` was deliberately left alone**, along with the Substack import source key. Those back real subscriber rows; only their display labels became MYMU. The Substack itself is still mindmakerlive.substack.com and its migration is Krish's.
- **A live defect fixed on the way.** `system_config.content_lane_techonomic` held `lane=mindmake, slot=investigation`, but the composer looks up `content_lane_{lane}_{slot}`, so `content_lane_mindmake_investigation` never resolved and the investigation slot silently fell back to a generic audience posture. The correctly-keyed row now exists.
- **The corpus collision rule is now two-way.** The VPS copy of `content-corpus/SKILL.md` had drifted and was titled *"1. Mindmake's publication: Teardowns (long-form investigation)"*, which is precisely the collision the rule forbids: section 1 precedes section 4 and would capture the weekly-channel lookup. Retitled to *Investigation (long-form teardown)* to match the DB copy, and both directions are now asserted programmatically (no heading may match both the investigation and the MYMU pattern). Because the format is called "MYMU: Teardown", the reverse mistake is now the easy one: never title the weekly section with "Teardown".
- **n8n.** Four workflows re-keyed (Orchestrator active, Content Factory / Marcus / Nova inactive). `VENTURE_TO_CHANNEL` now maps both `MINDMAKE` and `TECHONOMIC` to `makeyourmindup`. Verified after the write that zero nodes were renamed and there are zero dangling `$('node')` references or broken connections, since node names are string-referenced by expressions. The Orchestrator was confirmed still active afterwards. No git snapshot under `scripts/n8n/` contained the channel, so the sync-revert trap did not apply.

### 2026-08-06: Techonomic retired as a brand, folded into Mindmake LIVE

- **The ruling (Krish).** "techonomic.co will go, wire EVERYTHING to this new decision to fold everything techonomic in to Mindmake live." Principle: fewer brands is better. Techonomic's remit was a TOPIC, not an identity, and the depth engine built for it produces the public version of the advisory offer, so the proof belongs where the offer lives.
...
- **Alive as a format.** The length axis "Full (Techonomic)" is now **"Full essay"**, carrying the investigative steer (600-1000 words, decompose the claim, check each part against dated evidence, hold one counterpoint, say where the knowable ends, hard verdict). The `techonomic` venture rubric in `api/_finalPass.ts` became **`investigation`**, labelled "Mindmake LIVE (investigation)", keeping its five lenses and its instant-fail on an unverifiable load-bearing claim. The investigation pipeline (`api/_investigation.ts`, `api/_gates.ts`, `api/_harness.ts`, `api/investigations/*`) is renamed to Mindmake LIVE throughout and its evidence manifests are titled accordingly.
- **The bar follows the evidence, not the brand.** `api/content-ideas/[id]/final-pass.ts` now selects the `investigation` rubric whenever the row carries a verified evidence manifest (`meta.materials[].investigation_id`), whatever lane it sits in. Before this, the five-lens bar only fired for `lane='techonomic'`, so retiring the lane would have silently downgraded every future teardown to the lighter Mindmake LIVE rubric.
- **Nothing throws on an old row.** `normalizeLane()` reads the stored values `techonomic` and `publication` as Mindmake for lane filters, counts and cadence; `laneToFactoryChannel` / `laneToVenture` / `laneToCorpusChannel` / `save-draft`'s `laneToChannel` all map them onto Mindmake LIVE; `transform`, `synthesize`, `pilot/build-one`, `push-to-cleo`, `save-draft` and `briefs/:week/push` map a retired lane or channel in a request body instead of returning 400. `CadenceBar` renders nothing rather than throwing on a lane that no longer exists.
...
- **Not deleted, because it enforces the retirement.** Zara Signal Sweep's `const deadVentures=['adfixus','meliora']` guard and the `'TECHONOMIC': 'publication'` / `'ADFIXUS': 'linkedin'` aliases in the Orchestrator's `VENTURE_TO_CHANNEL` are what keep old briefs routing and retired ventures suppressed.
- **n8n.** Nine workflows cleaned in cloud. Zara Signal Sweep lost 35 nodes (three retired-venture scraping subgraphs), validated afterwards at 0 dangling edges and 0 broken `$('node')` references. The Omnichannel Content Factory's teardown branch was re-keyed off a `format` discriminator rather than the channel, so Mindmake LIVE still carries live builds and fresh news without every piece becoming a teardown. Node names containing the retired brand were deliberately left, since expressions reference them as strings.
...
- **CTRL B2C lane (Phase 1), public and L1-proven:** `/download` capture page live on ctrl.mindmake.co (mm-ctrl PR #333 + `VITE_FF_CAPTURE=true`); `capture-lead` edge function on CTRL's Supabase forwards to the new Acquisition workflows (Capture Intake, Nurture Scheduler daily 14:00 UTC, Unsubscribe); `acquisition_sends` L1 ledger + `acquisition_capture_to_paid` view + `acquisition_utm_conventions` config; Leo's Friday Pulse gained the funnel section. Full cycle verified with real email on the verified `mindmake.co` Resend domain before launch. Fixes en route: `leads_source_type_check` gained `capture`; `venture_registry` gained the missing `mm_ctrl` lane row (leads FK had blocked captures).
...
- **Doc drift corrected.** Live instance is **100 workflows / 90 active**, not the "~81 / ~76" the doc claimed (undercount of ~19). Folded in the entire missing **`Krish |`** inbox/objective group (6), `mind/make OS | RE Dossier Engine`, `Fleet | Attribution`, 4 extra Cleo + 3 extra Nell workflows; removed the phantom "Deep Enrich Retry Sweep" (no live workflow) and the double-counted Cleo "Capture Idea Webhook". Rebuilt the §3.4 table (sums to exactly 90).
...
- `System | mind/make OS | Audience Pipeline` (`7sYzU1FidUo2w1Lh`) sync tick: hourly -> every 3h (daily 07:30 reconcile unchanged).
- `System | mind/make OS | Critical Infrastructure Monitor` (`SXdHes0WwIovjPAB`): every 2h -> every 3h (the node name had long read '5m'; the live runtime was every 2h).
- `mind/make OS | RE Dossier Engine` (`aXvyxnTHAT1jjPet`): every 2h -> every 6h.
- `System | mind/make OS | Workflow Monitor` (`ceWoxAIadebfpxvh`): every 4h -> every 6h.
- `System | mind/make OS | Silent Success Detector` (`F6srw1yE9uH67q14`): every 4h -> every 8h.
- `Nova | mind/make OS | Visibility Sweeper` (`SIDlCqURzTVsVt70`) retry sub-trigger: every 6h -> every 12h (the weekly Mon 11:00 UTC sweep unchanged).
...
- **Lanes:** `signal_noise` (written, ~biweekly, AI×monetization in media/adtech/martech), `mindmake` (2/wk: `roundup` slot + `field_learning` slot), `techonomic` (weekly investigative, Perplexity-led), `builder_economy_ig` (daily, upbeat "built with AI", Krish posts manually to @the_builder_economy). BE *podcast* stays in the guest pipeline; this lane is Instagram only.
- **Schema (live + migration):** `content_ideas.lane/lane_slot/cadence_due_at`; `content_cadence` ledger (5 rows, RLS); `recompute_content_cadence()` fn. Migrations in `scripts/migrations/2026-06-02-*.sql`. 81 ideas backfilled (38 mindmake / 37 techonomic / 6 signal_noise).
...
Two new Cleo workflows shipped from Web Claude brief (mind/make OS content inspiration pipeline), executed by Claude Code on local Windows machine via the Supabase Management API + n8n MCP. Migration source-of-truth: `supabase/migrations/20260527160000_content_inspiration_pipeline_001.sql` in `krishanraja/control-center`.

- **New workflow `Cleo | mind/make OS | Inspiration Sweep`** (`D4W5TF1sP9lE828c`, 35 nodes, daily 06:00 UTC) - reads Gmail label `AI Newsletters` (full bodies, not snippets), exports Drive inspiration folder contents, loads `content_pillars` + cleo brief + thresholds, Sonnet 4.6 extracts publish-ready seeds against pillar bar (anti_patterns + evidence_required), Perplexity sonar-pro adds contrarian + adjacent stories per survivor, inserts `content_ideas` with `source_type=inspiration_sweep`. Zero rows is a valid output. Gemini 2.5 Pro fallback on Anthropic error.
- **New workflow `Cleo | mind/make OS | Synthesis Engine`** (`ES32WlTsgnr63qO3`, 28 nodes, Wed + Sun 12:00 UTC) - reads 14d of content_ideas + zara_signals + inspiration folder, Opus 4.7 hypothesizes 0-3 non-obvious angles connecting 3+ threads. Six-rule hard filter: 3+ threads, non-obvious, falsifiable, pillar-linked, confidence>=0.85, 3+ named entities. Inserts as `source_type=synthesis_hypothesis`. Zero rows is valid.
...
- **Old `Cleo | mind/make OS | Newsletter Sweep`** (`ZICpbmlXil11qMjs`) flagged for archive after 7 days of clean Inspiration Sweep runs. Was a triggerCount=0 sub-workflow with metadata-only Gmail reads (no bodies), name-only Drive reads (no contents), no pillar awareness, output went to Set node never to Supabase. Rename to `ZZ ARCHIVED | Cleo | Newsletter Sweep`.
...
- `Vera | mind/make OS | Feedback Aggregation` (`FZBDYXXfT1MBrAF6`) - zero rows in `workflow_runs` ever. Workflow is marked active but has never executed (or has never written a heartbeat - both equally broken from an observability standpoint).
- `Vera | mind/make OS | Behavioural Auditor` (`l0nujD2PBYGeEtXx`) - same. Zero runs ever.
