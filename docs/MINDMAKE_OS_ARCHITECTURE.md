...
**Video Engine and the Studio boundary.** The video and carousel engine is `krishanraja/mindmake-video-studio`, a deterministic renderer with a Windows runner; Control Center owns the control plane (`api/video-studio/*`, thirteen `video_studio_*` tables, the portable MCP gateway and the `mindmake_studio_*` learning spine). Editorial judgement lives here, not in the studio: the studio radar is a feed importer, and production starts from an approved `ProductionBriefV1` the runner claims. Reviews reach Krish through the mobile deck and the desktop obligation strip; learning proposals reach him in the Library. Activation state for the runner and the consumer OAuth connector is the ledger in `docs/plans/video-engine/STATE.md`.
