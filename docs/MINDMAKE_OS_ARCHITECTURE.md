...
> 2. **control-center GitHub repo:** `docs/MINDMAKE_OS_ARCHITECTURE.md`. **FILENAME SPLIT, fixed 2026-09-06:** the repo tracks `MINDMAKE_` (no R) while the VPS source and the skills are `MINDMAKER_` (with R). The sync script had been writing `MINDMAKER_OS_ARCHITECTURE.md` into the repo clone, a path git does not track, so **surface 2 silently diverged from the rebrand until 2026-09-06** and GitHub accumulated edits (a 2026-09-02 creator-scout entry) that never reached the other five. The script now targets the tracked `MINDMAKE_` filename and its name guard accepts both spellings. If you rename this file again, change `sync-architecture-surfaces.py` in the same commit or surface 2 goes dark with no error. The VPS clone at `/root/Projects/control-center/docs/...` is the same surface; the sync script writes that path directly, so it can be current while GitHub is behind. **Push, or GitHub silently lags.**
