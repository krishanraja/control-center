...
- Verify that an intact expired-slot preview receipt is acknowledged; under an exact fresh same-runner reclaim, verify that a retention-missing proxy is reproduced and re-uploaded through only its renewed immutable slot, while a changed binding, truncated object, or MD5 mismatch still fails closed.
