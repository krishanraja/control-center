...
| `/api/internal/sonnet-proxy` | Metered Anthropic bridge for n8n. Requires `Authorization: Bearer $N8N_PROXY_SECRET`; `X-Internal-Caller` is retained as the agent stamp. Model allowlist and output-token ceiling apply before the upstream call |
