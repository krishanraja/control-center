...
| `/api/meter/apify-sync` | Cron (`5 * * * *`) + POST. Pages `/v2/actor-runs` from a whole-UTC-day cursor, rolls `usageTotalUsd` up by actor × day × run origin, joins `apify_actor_registry` for `task_category`, and replaces those days in `meter_daily` (idempotent by construction). Also reads `/v2/users/me/usage/monthly` for the vendor's real cycle window onto `service_registry.cycle_*`, then evaluates the money lines and emails any crossing. `?days=N` (default 3, max 31). Reports `unregistered` — actors that ran but sit in no registry row |
